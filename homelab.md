...
<!-- 오버뷰 — 한 문장에 한 가지: 무엇을 구축했나(환경 둘 + 서비스 배포) → 왜 둘인가(dev 한계 → stg) → 어떻게 배포하나.
     마지막 문장이 배포로 끝나 바로 아래 배포 흐름 구성도로 넘어간다.
     5만 명 실측은 홈 리드와 부하 카드가 말하므로 여기서는 반복하지 않는다. -->

노트북 k3s의 **dev**와 AWS EKS의 **stg**, 두 환경을 구축해 대기열 예매 서비스를 배포했습니다.
dev는 개발 · 공개 데모 환경으로, 3만 명 부하에서 노드 한계에 도달해 prd 스펙 산정용 stg를 Terraform으로 구성했습니다.
