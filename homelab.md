...
만든 순서대로 다섯 갈래입니다. 무엇을 썼는지는 위 그림에 있으니, 여기서는 **왜 그렇게 정했는지**를 적었습니다.

<div class="hl-cards" markdown="0">
  <div class="hl-card">
    <span class="hl-tag">클러스터</span>
    <h3>기본으로 주는 것을 끄고 직접 골랐다</h3>
    <p>k3s는 네트워크·로드밸런서·인그레스를 기본으로 얹어 줍니다. 그걸 전부 끄고 Calico·MetalLB·Traefik으로 바꿨습니다 — 설정을 남이 쥐고 있으면 나중에 부하를 재도 손댈 곳이 없기 때문입니다.</p>
    <p>디스크는 워크로드마다 따로 잘라 붙였습니다. 한 볼륨을 나눠 쓰면 "누가 채웠는지"가 지표에 안 남습니다. kubelet이 쓸 몫도 짐작 대신 실측해서 예약했습니다.</p>
    <span class="hl-num">VM 3대 · etcd 3멤버 · 워크로드별 정적 PV 10장</span>
  </div>
  <div class="hl-card">
    <span class="hl-tag">배포</span>
    <h3>미는 대신 당겨가게 했다</h3>
    <p>배포 도구에 클러스터 자격을 쥐여 주는 대신, 클러스터가 저장소를 당겨가게 했습니다. 자격이 밖으로 나가지 않고, 저장소의 현재 상태가 곧 클러스터의 상태가 됩니다.</p>
    <p>이미지 태그는 고정값 대신 커밋마다 새 값을 붙입니다. 같은 태그를 덮어쓰면 무엇이 돌고 있는지 알 수 없고 되돌릴 수도 없습니다.</p>
    <span class="hl-num">push → 반영 3초 · 파이프라인 6분 6초 → 46초</span>
  </div>
  <div class="hl-card">
    <span class="hl-tag">관측</span>
    <h3>재기 전에 볼 눈부터 만들었다</h3>
    <p>부하를 걸기 전에 지표·로그·트레이스를 먼저 세웠습니다. 볼 눈이 없으면 판을 돌려도 "느렸다"까지만 알고 어디서 느렸는지는 못 잡습니다.</p>
    <p>세우는 동안 화면이 먼저 틀렸습니다. 시리즈가 상한에 차던 원인은 같은 프로세스를 두 곳에서 중복 수집한 것이었고, 노드 메모리 패널은 층이 다른 값을 섞어 빼고 있었습니다.</p>
    <span class="hl-num">중복 수집 제거 후 거절 0건</span>
  </div>
  <div class="hl-card">
    <span class="hl-tag">서비스와 용량 ★</span>
    <h3>정원은 정한 게 아니라 잰 값이다</h3>
    <p>대기열은 들어올 사람을 줄 세우고 정원만큼만 예매 화면에 들여보냅니다. 그 정원을 몇으로 둘지가 이 서비스의 전부인데, 처음에는 근거 없이 적어 둔 숫자였습니다.</p>
    <p>부하를 걸어 판을 거듭하자 병목이 앞에서 뒤로 옮겨 갔습니다 — 프록시 메모리, 대기열 CPU, DB 커넥션 풀, 캐시 폭주 순으로. 자리를 하나씩 따라가 정원·타임아웃·자원 스펙을 다시 정했고, 값 대신 <b>계수</b>를 남겼습니다. 사람이 늘면 무엇이 얼마나 느는지를 알아야 다음 규모를 계산할 수 있기 때문입니다.</p>
    <p>트레이스가 아니었으면 못 찾았을 것도 있습니다. 회차를 한 번 조회할 때 Redis를 스무 번 왕복하고 있었습니다.</p>
    <span class="hl-num">판 34회 · 동시 입장 1,000명 · 76만 요청에 5xx 0건</span>
  </div>
  <div class="hl-card">
    <span class="hl-tag">격리와 공개 ★</span>
    <h3>열되, 열린 자리를 세어 두었다</h3>
    <p>집에 공개하기 전에 클러스터를 방화벽 뒤 격리망으로 옮겼습니다. 관리 화면과 공개 서비스가 한 주소에 같이 있었고, 그 상태로 열면 둘이 같이 열리기 때문입니다.</p>
    <p>관리 통로는 VPN 하나만 남겼습니다. 키가 없으면 응답조차 하지 않아 문이 있다는 것도 밖에서는 보이지 않습니다. 공개 쪽은 Cloudflare를 거친 요청만 받게 해서, 집 주소를 알아도 우회로 들어올 수 없게 했습니다.</p>
    <p>격리망 안쪽도 평평하게 두지 않았습니다. 파드끼리 오갈 수 있는 통로를 하나씩 지정하고, 나머지는 막았습니다.</p>
