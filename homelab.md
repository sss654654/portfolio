...
title: 홈랩
description: >
  클라우드가 대신해 주던 것들을 노트북 한 대 위에 직접 세우고, 그 값들을 부하로 재서 정한 기록
permalink: /homelab/
---

<!-- ① 오버뷰 — 왜 이렇게 하기로 했나 → 무엇을 올렸나 → 그 결과가 아래 그림 -->

쿠버네티스를 EKS로 다루면 채용 공고가 요구하는 경험은 쌓입니다.
다만 **관리 포인트가 AWS로 넘어갑니다** — 컨트롤플레인은 AWS가 맡고 저는 워커 노드만 보게 되고,
EC2 한 대가 어떻게 서는지, 클러스터가 어떤 순서로 올라오는지는 콘솔 뒤에 가려집니다.
그 뒤를 직접 보고 싶었습니다.

그래서 집에 있던 노트북에 외장 SSD를 꽂아 Proxmox를 올렸습니다. 전기 말고는 비용이 들지 않아 두세 달을 내리 켜 두었습니다.

올릴 것은 정해져 있었습니다. 둘 다 **끝까지 가 보지 못한 것들**입니다.

* 부트캠프에서 5인 팀으로 만든 **예매 대기열 서비스** — EKS에 배포까지는 했지만 CI/CD와 클러스터 관측은 손대지 못했습니다
* 회사에서 맡은 **옵저버빌리티** — 애플리케이션은 요청·에러·지연으로, k3s 인프라는 사용률·포화로 봤지만, 이미 서 있는 플랫폼 위에 대시보드를 얹은 것이었습니다

이번에는 그 아래를 직접 만들었습니다. 물리 디스크와 리눅스에서 시작해 클러스터·배포·관측·부하 실측·격리까지 한 층씩 올라왔고,
...
그림 한 장에 담기지 않는 것이 **각 층에서 무엇을 왜 그렇게 정했는가**입니다.
만든 순서대로 다섯으로 나눠, 각각을 따로 정리했습니다.
