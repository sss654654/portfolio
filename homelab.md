...
관리형 쿠버네티스는 밑단을 대신 맡아 주는 만큼, 그 안이 어떻게 도는지는 보여주지 않습니다.
EKS라면 컨트롤플레인은 AWS 몫이고, 노드 아래의 리눅스와 네트워크는 만질 일이 없습니다.
이 홈랩은 **그 가려진 층부터 직접 쌓아 보려고** 시작했습니다.

올린 것은 끝까지 가 보지 못한 두 경험입니다 — 배포까지만 해 봤던 예매 대기열 서비스,
그리고 이미 세워진 플랫폼 위에서만 해 봤던 관측.
이번에는 물리 디스크와 리눅스부터 직접 쌓았고, 올린 뒤에는 부하를 걸어
정원과 자원 값을 짐작 대신 실측으로 정했습니다.

그 결과가 지금 [ticket.subinhong.dev](https://ticket.subinhong.dev) 에서 돌고 있습니다. 구성은 이렇습니다.
...
구성도가 말해 주지 않는 것은 **각 층에서 무엇을 왜 그렇게 정했는가**입니다.
만든 순서대로 다섯으로 나눠 정리했습니다.
