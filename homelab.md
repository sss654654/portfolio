...
title: 홈랩
description: >
  노트북 한 대 위에 k3s 클러스터를 세우고 예매 대기열 서비스를 공개해 운영합니다
permalink: /homelab/
---

<!-- ① 오버뷰 — 왜 이렇게 하기로 했나 → 무엇을 올렸나 → 그 결과가 아래 그림 -->

관리형 쿠버네티스는 컨트롤플레인을 맡아 주고, 파드 네트워크·로드밸런서·볼륨도 관리형 컴포넌트가 대신 만들어 줍니다.
그 자리를 직접 골라 세워 보고 싶었고, **그 아래 하이퍼바이저와 리눅스, 디스크까지 내려가 보려고** 홈랩으로 시작했습니다.

그 위에 올릴 것도 정해져 있었습니다 — 배포까지만 해 본 예매 대기열 서비스,
그리고 클러스터는 이미 서 있는 상태에서 세웠던 관측.
이번에는 직접 만든 클러스터에 서비스를 올리고, 관측도 그 위에 다시 세웠습니다.

정원과 자원 값은 짐작으로 두지 않고 부하를 걸어 정했습니다.
그렇게 만든 것이 지금 [ticket.subinhong.dev](https://ticket.subinhong.dev) 에서 아래 구성으로 돌고 있습니다.
