...
인프라를 직접 구축하고, 지표로 원인과 한계를 판단하는 인프라 엔지니어입니다.
기구축 k3s 환경의 **옵저버빌리티**를 담당했고, 이후 **가상화부터 Terraform 기반 AWS까지** 인프라를 직접 구성했습니다.
문제는 metric · log · trace로 원인을 좁히고, 용량은 SLO를 먼저 정한 **부하 테스트로 실측**합니다.
{:.lead}
