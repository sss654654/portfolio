...
노트북 한 대로 쿠버네티스 · CI/CD · 옵저버빌리티 · 방화벽을 세우고, 그 위에 대기열 예매 서비스를 인터넷에 운영하며 **10,000명 부하 테스트를 에러 없이 통과**했습니다.
회사에서는 k3s 옵저버빌리티를 구축하고, profile로 찾은 함수를 개발팀이 고쳐 **메모리를 78%** 줄였습니다.
부트캠프에서는 대기열 백엔드를 맡고 **AWS 개발계 네트워크를 Terraform으로 구성**했습니다.
{:.lead}
...
* **2022.07 – 12** — [LevelDB 캐시 메커니즘 분석](/projects/leveldb/) — 학부 연구생 · **KSC 2022 논문 1저자**
{:.timeline}
