...
쿠버네티스 환경을 구축하고, 측정한 값으로 판단하는 인프라 엔지니어입니다.
기구축 k3s 환경에서 **옵저버빌리티**를 담당했고, 이후 **가상화부터 Terraform으로 구성한 AWS까지** 한 층씩 직접 세웠습니다.
원인은 metric · log · trace로 좁히고, 용량은 SLO를 먼저 정한 **부하 테스트로 실측**합니다.
{:.lead}
