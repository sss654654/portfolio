...
| | 기본값 | 이 홈랩 | 그렇게 한 이유 |
|---|---|---|---|
| 하이퍼바이저 | Windows 위에 얹는 Type&nbsp;2 | **Proxmox** — Type&nbsp;1 | 호스트 OS가 자원을 상시 차지하지 않아, 노트북 자원을 전부 VM에 배분할 수 있음 |
| VM 메모리 | ballooning — Proxmox가 부족하면 VM RAM을 도로 회수 | **노드마다 8GB 통째로 고정** | 쿠버네티스는 노드 RAM을 고정으로 보고 파드를 앉힘 — 도중에 줄면 그 계산이 틀어짐 |
| 절전 | 뚜껑 닫으면 잠자기 · USB 자동절전 | **둘 다 차단** | 서버는 뚜껑을 닫아 두고, 루트 디스크가 외장 USB라 재워지면 파일시스템이 끊김 |
{:.hl-cmp}
...
| | 기본값 | 이 홈랩 | 그렇게 한 이유 |
|---|---|---|---|
| 파드 네트워크 | Flannel | **Calico** | Flannel은 트래픽만 나르고 정책은 집행하지 않음 — 네임스페이스 사이를 막으려면 집행할 주체가 필요함 |
| 로드밸런서 | ServiceLB — 노드 IP를 빌림 | **MetalLB** `10.0.0.240-250` | 서비스에 노드와 별개인 주소를 붙여야 밖에서 부를 대표 주소 하나가 생김 |
| 인그레스 | k3s가 같이 깔아 주는 Traefik | **직접 올린 Traefik** | 공개 443과 관리 80을 나눠 쓰려면 설정을 고쳐야 하는데, k3s가 깔아 준 쪽은 고쳐도 업그레이드 때 되돌아감 |
| 스토리지 | local-path — 한 파일시스템에 폴더로 | **정적 PV 10장** — 위 그림의 디스크를 그대로 | 사용량 지표가 **파일시스템 단위**(`statfs`)로만 나와서, 자르지 않으면 부하 때 무엇이 채웠는지 못 가림 |
{:.hl-cmp}
...
| 이슈 | 원인 | 조치 |
|---|---|---|
| 노드가 가진 메모리를 전부 파드 몫으로 신고 — 그만큼 과배치된다 | k3s는 API 서버·etcd를 파드가 아닌 프로세스로 돌려 kubelet 집계 밖 | 예약값 설정. 워크로드를 다 올린 뒤 실측 **1783Mi**로 1Gi → 2Gi 수정, 세 노드 반영해 allocatable 5081Mi |
