...
쓰던 노트북 한 대에 RAM 32GB. 내장 Windows를 지우지 않기로 해서 서버는 외장 디스크로 부팅합니다.
**관리형이 만들어 주던 네트워크·로드밸런서·볼륨은 여기 없어, 층마다 기본값을 직접 골라야 했습니다.**
...
| 파드 네트워크 | Flannel — 전달만 | **Calico** | Flannel은 NetworkPolicy를 집행하지 않음 — 규칙을 적어도 통신이 그대로 열림 |
| 로드밸런서 | ServiceLB — 노드 IP를 빌림 | **MetalLB** `10.0.0.240-250` | 빌린 노드가 멈추면 그 주소도 함께 사라짐 |
| 인그레스 | k3s가 같이 깔아 주는 Traefik | **직접 올린 Traefik** | k3s가 애드온으로 관리해 고친 설정이 재기동마다 원복 — 직접 쥐려면 관리 밖으로 |
...
- **네트워크·로드밸런서·인그레스·스토리지가 고른 것으로 섰습니다** — Calico · MetalLB `10.0.0.240-250` · 직접 올린 Traefik · 노드 디스크를 그대로 쓰는 정적 PV 10장
...
- **외장 USB SSD가 단일 장애점입니다** — 케이블이 빠지면 서버가 내려갑니다. Windows를 지킨 대가라 없앨 수 없고, 자동절전 차단과 종료 절차 고정으로 끊길 경로를 줄이는 데까지 했습니다
