...
| VM 메모리 | ballooning — Proxmox가 부족하면 VM RAM을 도로 회수 | **노드마다 8GB 통째로 고정** | 쿠버네티스가 RAM을 고정으로 보고 파드를 앉힘 |
| 절전 | 덮개를 닫으면 잠자기 · USB 자동절전 | **둘 다 차단** | 잠들면 서버가 그대로 멈춤. 루트 디스크가 외장 USB라 절전되면 끊김 |
{:.hl-cmp}
...
| | 기본값 | 이 홈랩 | 그렇게 한 이유 |
|---|---|---|---|
| 배포판 | 표준 k8s — 컴포넌트를 따로 세움 | **k3s** — 단일 바이너리 | 컨트롤플레인이 가벼울수록 세 노드가 나눠 쓰는 RAM에서 서비스 몫이 남음. 대신 컴포넌트 넷이 한 프로세스라 하나가 죽으면 같이 죽음 — HA를 노드 단위(3대)로 만든 이유 |
| 파드 네트워크 | Flannel — 전달만 함 | **Calico** | 기본은 모든 파드가 서로 닿음. 막으려면 NetworkPolicy가 필요한데 Flannel은 집행하지 않음 |
| 로드밸런서 | ServiceLB — 노드 IP를 빌림 | **MetalLB** `10.0.0.240-250` | 노드 IP로 부르면 그 노드가 죽을 때 주소도 죽음. 어느 노드가 살아 있든 같은 주소로 들어오게 함 |
| 인그레스 | k3s가 같이 깔아 주는 Traefik | **직접 올린 Traefik** | 공개 443과 관리 80을 나누려면 설정을 고쳐야 하는데, k3s가 관리하는 쪽은 k3s를 올릴 때마다 원래대로 덮어써짐 |
| 스토리지 | local-path — 한 파일시스템에 폴더로 | **정적 PV 10장** — 노드에 붙인 디스크 그대로 | 사용량 지표가 **파일시스템 단위**(`statfs`)로만 나옴 — 안 자르면 무엇이 채웠는지 못 가림. 대신 그 디스크를 쓰는 파드는 그 노드에 묶임 |
{:.hl-cmp}
...
| 증상 | 원인 | 조치 |
|---|---|---|
| 데스크탑에서 `kubectl`·SSH·Proxmox 웹UI가 전부 무응답 | Intel e1000e NIC의 세그멘테이션 오프로드 결함. 링크는 살아 있는데 ARP만 실패하고 `Hardware Unit Hang` **34회** | 그 기능을 끄고 CPU가 처리하게 함 |
| 나흘 뒤 재발 | 자동 적용 규칙의 조건을 NIC 최종 이름으로 걸었는데, 그 이름은 부팅 중에 붙음 — **조건이 맞는 순간이 없어 한 번도 실행되지 않음** | 적용 시점을 이름이 확정된 뒤로 옮기고, 적용·재기동·재부팅을 각각 확인 |
| 노드가 신고한 파드 몫이 실제 여유보다 큼 | k3s는 컨트롤플레인을 파드가 아니라 리눅스 프로세스로 돌리는데 kubelet은 파드만 셈 — 그 몫 **1783Mi**가 집계에서 빠짐 | 신고에서 미리 뺌 — k3s **2Gi** · OS **512Mi** |
{:.hl-tbl}
...
- 노드가 파드에 내줄 수 있는 몫이 **노드당 5081Mi**로 정해졌습니다 (7941 − 2048 − 512 − 300)
- 덮개를 닫아도, USB가 유휴여도 **잠들지 않습니다**

## 남은 것

**외장 USB SSD가 단일 장애점입니다.** 케이블이 빠지면 서버가 그대로 내려갑니다.
내장 Windows를 지우지 않으려는 선택의 대가이고 물리 제약이라 없앨 수 없어,
자동절전을 막고 종료 절차를 고정해 끊길 수 있는 경로를 줄이는 데까지만 했습니다.
