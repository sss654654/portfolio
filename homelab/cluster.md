...
| 노드가 가진 메모리 전부를 파드 몫으로 신고 — k3s-server가 이미 **1783Mi**를 쥐고 있는데도 | k3s는 컨트롤플레인을 파드가 아니라 리눅스 프로세스로 돌리고 kubelet은 파드만 셈 — 메모리가 마르면 커널이 그 프로세스를 죽임 | 실측해 약 **2.5GB**를 신고에서 뺌 (`kube-reserved` 2Gi · `system-reserved` 512Mi) |
{:.hl-tbl}
