...
| 인그레스 | k3s가 같이 깔아 주는 Traefik | **직접 올린 Traefik** | 공개 443과 관리 80을 나누려면 설정을 고쳐야 하는데, k3s가 관리하면 재기동마다 원복 |
...
| 노드가 신고한 파드 몫이 실제 여유보다 큼 | k3s는 컨트롤플레인을 파드가 아닌 리눅스 프로세스로 돌리는데 kubelet은 파드만 집계 — **1783Mi** 누락 | 신고에서 미리 뺌 — k3s **2Gi** · OS **512Mi** |
{:.hl-tbl}
...
Windows를 지킨 대가라 없앨 수는 없고, 자동절전 차단과 종료 절차 고정으로
