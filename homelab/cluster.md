...
| 증상 | 원인 | 조치 |
|---|---|---|
| 데스크탑에서 `kubectl`·SSH·Proxmox 웹UI가 전부 무응답. 호스트 콘솔은 로그인되고, 호스트에서 VM은 닿는데 공유기만 안 닿음 | 링크는 살아 있는데(`carrier yes`) ARP 실패. Intel e1000e NIC의 세그멘테이션 오프로드 결함 — `Hardware Unit Hang` 34회 | 해당 오프로드 비활성화 |
| 나흘 뒤 같은 증상 재발 | 재부팅 대비로 넣은 udev 규칙이 한 번도 실행되지 않음 — 조건을 NIC의 최종 이름으로 걸었는데 장치 이벤트 시점 이름은 아직 `eth0` | 인터페이스 기동 직후로 이동. 적용·재기동·재부팅을 각각 확인 |
| 파드를 다 올린 뒤 재 보니, 노드는 가진 메모리 전부를 파드 몫으로 신고하는데 k3s-server가 이미 **1783Mi**를 쥐고 있음 | k3s는 API 서버·etcd·스케줄러를 파드가 아니라 리눅스 프로세스로 돌리고, kubelet은 파드만 세므로 그 몫이 집계 밖 | 실측해서 `kube-reserved` 2Gi · `system-reserved` 512Mi — 약 2.5GB를 신고에서 뺌 |
{:.hl-tbl}

앞의 둘은 **넣은 것과 도는 것이 달랐고**, 마지막은 **신고한 것과 실제가 달랐습니다.**
예약을 안 하면 메모리가 마를 때 커널이 가장 큰 프로세스를 죽이는데, 그 후보가 etcd를 안은 k3s-server입니다.
