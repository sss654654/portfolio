...
| 항목 | 기본값 | 선택 | 이유 |
|---|---|---|---|
| 배포판 | 표준 쿠버네티스 — 컴포넌트를 따로 세움 | **k3s** — 단일 바이너리 | 컨트롤플레인이 가벼워야 8GB 노드에 서비스 몫 확보 · 대가는 한 프로세스라 동반 종료 |
...
| 스토리지 | local-path — 한 파일시스템에 폴더로 | **정적 PV 10장** — 노드에 붙인 디스크 그대로 | 디스크 metric이 **파일시스템 단위** — 안 자르면 무엇이 채웠는지 식별 불가 · 대가는 파드가 노드에 고정 |
{:.hl-cmp}
...
| 노드가 신고한 파드 몫이 실제 여유보다 큼 | 컨트롤플레인이 파드가 아닌 프로세스라 kubelet 집계에서 빠짐 — **1783Mi** | 신고에서 미리 뺌 — k3s **2Gi**(여유 포함) · OS **512Mi** |
{:.hl-tbl}
...
- 파드에 내줄 몫이 **노드당 5081Mi**로 정해졌습니다 — 노드 메모리 7941Mi에서 k3s 2048 · OS 512 · eviction 300을 뺀 값
- 덮개를 닫아도 잠들지 않고, 입출력이 없어도 USB 디스크 전원이 **내려가지 않습니다**
- **k3s 설치는 `config.yaml`, 그 위 부트스트랩(Calico부터 ArgoCD까지 9단계)은 스크립트, 그 위는 Helm 차트로 정의돼 있습니다**

## 한계

- **외장 USB SSD가 단일 장애점입니다** — 케이블이 빠지면 서버가 정지합니다. Windows를 유지하는 한 없앨 수 없어, 자동절전 차단과 종료 절차 고정으로 대응했습니다
- **Proxmox와 VM 생성은 코드가 아닙니다** — 하이퍼바이저 설치와 VM 3대는 손으로 만들었고, 코드는 그 위 k3s부터입니다
