...
| 증상 | 원인 | 조치 |
|---|---|---|
| 데스크탑에서 클러스터 전체가 안 닿음. 오프로드를 끄니 풀렸다가 나흘 뒤 재발 | Intel e1000e NIC 결함으로 NIC이 멈춤(`Hardware Unit Hang`). 재부팅 대비로 넣은 udev 규칙은 조건을 NIC의 최종 이름으로 걸어 한 번도 실행되지 않음 | 오프로드를 끄고, 그 설정을 인터페이스 기동 직후로 옮겨 재부팅을 넘게 함 |
| 파드가 0개인데 노드가 메모리 전부를 파드 몫으로 신고 | k3s는 API 서버·etcd를 파드가 아닌 프로세스로 돌려 kubelet 집계 밖 | 예약 **2Gi**. 처음 1Gi는 추정이었고, 워크로드를 올린 뒤 실측 **1783Mi**를 보고 올림 |
| 설치는 성공했는데 노드 라벨만 다르게 붙음 | Windows에서 체크아웃한 스크립트의 CRLF가 라벨 값 끝에 섞임 | 저장소에 줄바꿈·실행 권한 고정. 스크립트에 라벨 값 확인 절차 추가 |
{:.hl-tbl}

세 건 다 겉으로는 정상이었습니다 — **링크는 살아 있었고, 신고는 올라갔고, 설치는 성공했습니다.**
