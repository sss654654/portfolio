...
| | 기본값 | 이 홈랩 | 그렇게 한 이유 |
|---|---|---|---|
| 배포판 | 표준 k8s — 컴포넌트를 따로 세움 | **k3s** — 단일 바이너리 | 컨트롤플레인이 가벼워야 8GB 노드에 서비스 몫이 남음. 대가: 넷이 한 프로세스라 같이 죽음 — 그래서 HA는 노드 3대 |
| 파드 네트워크 | Flannel — 전달만 함 | **Calico** | 쿠버네티스 기본은 모든 파드가 서로 닿음 — 막는 NetworkPolicy를 Flannel은 집행하지 않음 |
| 로드밸런서 | ServiceLB — 노드 IP를 빌림 | **MetalLB** `10.0.0.240-250` | 노드 IP로 부르면 그 노드가 죽을 때 주소도 같이 죽음 — 주소가 노드에서 떨어져 있어야 함 |
...
Windows를 지키는 선택의 대가라 없앨 수는 없고 — 자동절전 차단과 종료 절차 고정으로
끊길 경로를 줄이는 데까지만 했습니다.
