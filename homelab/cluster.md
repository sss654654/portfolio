...
| 노드가 신고한 파드 몫이 실제 여유보다 큼 — 그대로 두면 메모리가 마를 때 커널이 k3s-server를 죽임 | k3s는 컨트롤플레인을 파드가 아니라 리눅스 프로세스로 돌리고, kubelet은 파드만 세므로 그 몫 **1783Mi**가 집계에서 빠짐 | 가장 큰 노드 값에 여유를 더해 신고에서 미리 뺌 — k3s **2Gi** · OS **512Mi** |
{:.hl-tbl}
