...
| 파드 네트워크 | Flannel — 전달만 함 | **Calico** | 네임스페이스 사이를 막을 NetworkPolicy 집행 주체가 필요 |
| 로드밸런서 | ServiceLB — 노드 IP를 빌림 | **MetalLB** `10.0.0.240-250` | 노드에 묶이지 않은 대표 주소 하나가 필요 |
| 인그레스 | k3s가 같이 깔아 주는 Traefik | **직접 올린 Traefik** | 공개 443과 관리 80을 나누려면 설정을 고쳐야 하는데, k3s가 깐 쪽은 업그레이드 때 되돌아감 |
| 스토리지 | local-path — 한 파일시스템에 폴더로 | **정적 PV 10장** — 위 그림의 디스크 그대로 | 사용량 지표가 **파일시스템 단위**(`statfs`)로만 나옴 — 안 자르면 무엇이 채웠는지 못 가림 |
{:.hl-cmp}
...
| 증상 | 원인 | 조치 |
|---|---|---|
| 데스크탑에서 `kubectl`·SSH·Proxmox 웹UI가 전부 무응답 | 큰 데이터를 쪼개는 일을 NIC에 맡기는 기능(세그멘테이션 오프로드)에 Intel e1000e 결함 — 링크는 살아 있는데 ARP만 실패하고 `Hardware Unit Hang` 34회 | 그 기능을 끄고 CPU가 쪼개게 함 |
| 나흘 뒤 같은 증상 재발 | 이 NIC은 부팅 중 `eth0`에서 최종 이름으로 바뀌는데, 자동 적용 규칙의 조건을 최종 이름으로 걸어 **조건이 맞는 순간이 없었음** — 한 번도 실행된 적 없음 | 적용 시점을 이름이 확정된 뒤로 옮기고, 적용·재기동·재부팅을 각각 확인 |
| 노드가 신고한 파드 몫이 실제 여유보다 큼 — 그대로 두면 메모리가 마를 때 커널이 k3s-server를 죽임 | k3s는 컨트롤플레인을 파드가 아니라 리눅스 프로세스로 돌리고, kubelet은 파드만 세므로 그 몫 **1783Mi**가 집계에서 빠짐 | 그 몫을 신고에서 미리 뺌 — k3s **2Gi** · OS **512Mi** |
{:.hl-tbl}

## 결과

<dl class="hl-stats hl-stats-row" markdown="0">
  <div><dt>3대</dt><dd>control-plane 겸 etcd — 한 대가 멈춰도 유지</dd></div>
  <div><dt>14.9Gi</dt><dd>파드에 내줄 수 있는 총량</dd></div>
  <div><dt>10개</dt><dd>워크로드마다 갈린 파일시스템</dd></div>
</dl>

노드당 5081Mi는 물리 7941Mi에서 k3s 2Gi, OS 512Mi, 퇴출 기준선 300Mi를 뺀 값이고,
이 상태로 뚜껑을 닫은 채 상시 가동합니다.
