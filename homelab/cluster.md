...
| 노드 예약 | 없음 | **2Gi** | k3s 몫이 kubelet 집계 밖이라 사람이 빼 줘야 함 |
{:.hl-cmp}

노드 예약은 설계할 때 예상하지 못한 자리였습니다.

```
k3s      API 서버·etcd·스케줄러를 파드가 아니라 프로세스 하나로 돌린다
kubelet  파드만 세므로 그 몫은 집계 밖 — 가진 메모리 전부를 파드 몫으로 신고
리눅스   메모리가 바닥나면 커널이 큰 프로세스부터 죽인다 — etcd 를 안은 k3s
조치     kube-reserved 로 그 몫을 신고에서 미리 뺀다
실측     파드 0개일 때 450Mi 를 재고 1Gi 로 잡음. 워크로드를 올린 뒤 1783Mi → 2Gi
```

## 트러블슈팅

어느 날 데스크탑에서 클러스터 전체가 안 닿았습니다.

```
증상   kubectl · SSH · Proxmox 웹UI 가 전부 무응답
진단   호스트 콘솔은 로그인됨. 호스트→VM 은 닿고 호스트→공유기만 안 닿음
       링크는 살아 있는데(carrier yes) ARP 실패 — 선이 아니라 그 위에서 멈춤
원인   Intel e1000e NIC 의 세그멘테이션 오프로드 결함 · Hardware Unit Hang 34회
조치   해당 오프로드 비활성화

재발   나흘 뒤 같은 증상
원인   재부팅 대비로 넣은 udev 규칙이 한 번도 실행되지 않음 —
       조건을 NIC 의 최종 이름으로 걸었는데 장치 이벤트 시점 이름은 아직 eth0
조치   인터페이스 기동 직후로 이동. 적용 · 재기동 · 재부팅을 각각 확인
```

재발 전까지 규칙은 넣어 둔 상태였습니다. **넣은 것과 도는 것은 다른 사실이었습니다.**
