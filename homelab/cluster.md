...
| 증상 | 원인 | 조치 |
|---|---|---|
| 데스크탑에서 `kubectl`·SSH·Proxmox 웹UI가 전부 무응답 | Intel e1000e NIC의 세그멘테이션 오프로드 결함 — 링크는 살아 있는데(`carrier yes`) ARP가 실패하고 `Hardware Unit Hang` 34회 | 해당 오프로드 비활성화 |
...
| 파드를 다 올린 뒤에도 노드가 가진 메모리 전부를 파드 몫으로 신고 — k3s-server는 이미 **1783Mi**를 쥔 상태 | k3s는 API 서버·etcd·스케줄러를 파드가 아니라 리눅스 프로세스로 돌리고 kubelet은 파드만 세므로 그 몫이 집계 밖. 이대로 메모리가 마르면 커널이 가장 큰 프로세스인 k3s-server를 죽임 | 실측해서 `kube-reserved` 2Gi · `system-reserved` 512Mi — 약 2.5GB를 신고에서 뺌 |
{:.hl-tbl}

세 건 다 겉으로는 정상이었습니다 — **링크는 살아 있었고, 규칙은 들어가 있었고, 신고는 올라갔습니다.**
