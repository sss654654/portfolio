...
| 증상 | 원인 | 조치 |
|---|---|---|
| **데스크탑에서 격리망을 부르면 답이 안 옴**<br>방화벽 로그에는 초록 pass가 찍힘 | **가는 길은 열렸는데 오는 길이 돌아감**<br>WAN 쪽 규칙에 `reply-to`가 자동으로 붙음 — 같은 대역이라 바로 건넬 응답이 공유기로 나가 버려짐 | **그 규칙에서 `reply-to` 해제**<br>응답이 같은 대역으로 바로 오면서 손실 **0%** |
