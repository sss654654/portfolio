...
클러스터를 이루는 VM 세 대가 데스크탑과 같은 사설망에 있었습니다. 인터넷에서 들어오는 경로가
...
| 증상 | 원인 | 조치 |
|---|---|---|
| **방화벽에 주소를 넣었는데 밖에서 응답 없음**<br>콘솔 메뉴에는 `.210` 이 찍혀 있음 | **설정 파일에 저장만 되고 커널에는 안 붙음**<br>`ifconfig` 의 실제 주소는 DHCP 잔존값 `.191` — 그 주소인 기기가 없으니 무응답이 맞는 동작 | **재부팅으로 설정 파일 기준 재적용**<br>ARP 응답이 잡히고 `.210` 으로 통신 |
| **정책을 건 뒤 Grafana 접속이 타임아웃**<br>Traefik 까지는 도달 | **정책 포트를 Service 값으로 적음**<br>Service 는 80 인데 파드는 3000 을 염 — 정책은 kube-proxy 가 주소를 바꾼 뒤 파드 포트로 판정 | **3000 으로 교체**<br>`targetPort` 를 이름으로 적은 자리도 전부 숫자로 |
