...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| 격리 방식 | **OPNsense 방화벽 VM** + 가상 브리지로 사설망 구축 | **한 공유기 아래 노드와 개발 PC 가 나란히 인터넷을 향한다**<br>어느 한쪽이 침해되면 같은 망을 타고 서로에게 건너간다 |
| 관리 접근 | **WireGuard** (OpenVPN 아님) | **관리 페이지는 공개 대상이 아니다**<br>클러스터가 방화벽 뒤로 가면서 Grafana·ArgoCD 를 도메인으로 열지 않고, 키를 등록한 관리자만 터널로 들어오게 했다 |
