...
<figcaption>dev · stg 공통 구조 — 원본 저장소만 dev MinIO · stg S3로 다름. exporter를 붙일 수 없는 RDS · ElastiCache · ALB는 stg의 CloudWatch exporter가 Mimir로 수집.</figcaption>
</figure>
...
| 원본 저장소 | **dev MinIO 파드 · stg S3(IRSA)** | 노드 디스크엔 WAL만 — 나머지는 오브젝트 스토리지에서 재생성 · stg는 IRSA로 키 없이 접근 |
...
| 증상 | 원인 | 조치 |
|---|---|---|
| dev 활성 시리즈 상한 15만 도달 — 오류 표시 없이 metric **값만 누락** | k3s는 `:6443` · `:10250`이 **같은 프로세스** — 둘 다 수집하면 중복 metric이 상한 15만의 85% 차지 | `:6443` 수집 제거 · 상한 30만 → 초과 거절 **0** |
