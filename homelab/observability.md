...
- 그 위에 **판 둘이 완성**됐습니다 — 클러스터 안을 보는 판과, 그 아래 물리 호스트(Proxmox)를 보는 판
- 안 볼 때는 **알림이 Discord로** 옵니다 — 지금 값과 할 일, 그 패널의 그림과 함께
- 판·알림·배선이 **전부 git에** 있습니다 — 화면에서 고친 것은 재시작하면 사라지는 구조라, 바뀌는 길이 커밋 하나뿐입니다

## 남은 것

- **관측 스택 자신을 보는 판이 남았습니다** — 지표가 조용히 버려지는 일을 겪었는데, 그 계열 지표의 이해와 전용 대시보드는 아직 숙제입니다
- **설계한 request를 넘거나 limit에 다가선 파드들이 있습니다** — 노드 메모리(8GB×3)의 한계라 전부에게 넉넉히 줄 수 없습니다. 자원이 큰 클러스터로 옮길 때, 이 판들로 잰 값으로 스펙을 다시 설계합니다
