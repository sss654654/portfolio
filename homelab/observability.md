...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| 지표 저장소 | **Mimir distributed** — ingester만 3대, 노드당 1 | 최근 2시간이 ingester 메모리에만 있어 죽으면 그 구간 소실 — 셋 다는 못 하니 **알림과 대시보드가 선 지표에만** |
| 로그·트레이스 | **Loki·Tempo는 단일** | 위험은 같아도 조사 도구라 **비어도 판정에 무영향** · WAL로 재시작만 복구, 노드째 유실은 감수 |
| 저장 몸통 | **클러스터 안 MinIO** — S3 호환 | 셋 다 원본은 여기, 로컬엔 WAL만 · 전용 계정이 세 버킷만 접근 — 루트 자격은 앱에 미노출 |
...
<div class="hl-shots" markdown="0" aria-label="클러스터 인프라 대시보드 — 화살표로 넘겨 봅니다">
  ...
  <figure class="hl-shot">
    ...
    <figcaption><b>(행1 · 노드 메모리)</b> 왼쪽은 스케줄러 기준(선언만 더해 새 파드가 앉을 자리),
    오른쪽은 커널 기준(실제로 쓰고 남은 양)입니다. 축이 달라 표를 갈랐고
  ...
  <figure class="hl-shot">
    ...
    <figcaption><b>(행4 · 파드 상태)</b> 위는 흔적 — 비정상이었던 파드(OOMKilled·Pending)와
    재시작이 잦아진 것이 남습니다. 아래는 임박 — 메모리가 limit의 80%를 넘거나
    CPU가 5분 평균으로 limit에 닿은 컨테이너입니다.</figcaption>
  </figure>
  <figure class="hl-shot">
  ...
  <figure class="hl-shot">
    ...
    <figcaption><b>(행5 · 스펙 장부)</b> 도는 컨테이너 전수의 선언 대 실사용입니다. 빨강은
    request를 넘겨 쓰는 것(몰리면 먼저 축출), 파랑은 선언만 하고 안 쓰는 것 —
    스펙 변경은 이 화면에서 시작합니다.</figcaption>
  </figure>
</div>

## 호스트 대시보드와 알림

물리 노트북은 호스트 대시보드가, 화면 밖의 시간은 **알림**이 맡습니다.
...
<div class="hl-shots" markdown="0" aria-label="호스트 하드웨어 대시보드와 Discord 알림 — 화살표로 넘겨 봅니다">
  ...
  <figure class="hl-shot">
    ...
    <figcaption><b>(행1 · 열)</b> 왼쪽이 증상(온도), 가운데가 원인 축(클럭 — powersave로 평시를
    낮추고 최고 코어 4.3GHz 유지), 오른쪽이 범위(NVMe — 섀시까지 뜨거운가)입니다.
    90°C 스파이크는 알림의 5분 지속 조건이 거릅니다.</figcaption>
  </figure>
  <figure class="hl-shot">
  ...
  <figure class="hl-shot">
    ...
    <figcaption><b>(행4 · 저장)</b> 외장 SSD 한 장을 세 각도로 봅니다 — Proxmox 몫(local),
    VM 몫(local-lvm — 사용량과 한도 점선), 물리 건강(온도·읽기 오류).
    VM 몫은 기본 지표에 없어 <code>lvs</code>를 읽는 수집기를 붙였습니다.</figcaption>
  </figure>
  <figure class="hl-shot">
  ...
  <figure class="hl-shot">
    ...
    해소(77°C). 지금 값과 조치, 패널 그림이 함께 옵니다. 알림을 고른 기준은 둘 —
    오면 일어나서 할 일이 있나, 안 울리면 되돌릴 수 없나.</figcaption>
  </figure>
</div>
...
- **세 신호가 각자의 저장소에 쌓입니다** — 지표 Mimir(ingester 3대 · 15일) · 로그 Loki(7일) · 트레이스 Tempo(24시간). 원본은 셋 다 MinIO로 내려갑니다
- **대시보드 둘을 세웠습니다** — 클러스터 상태와 호스트 하드웨어(열·전력·저장). 알림은 호스트 쪽에만 걸었습니다
...
- **request를 넘거나 limit에 다가선 파드가 아직 있습니다** — 노드 메모리 8GB × 3 안에서는 전부에게 여유를 못 줍니다. 자원이 큰 클러스터에서 다시 잡습니다
