...
클러스터도 배포 경로도 섰지만, 안에서 무슨 일이 벌어지는지 알 방법이 없었습니다 —
관측은 **세워 놓는다고 도는 것이 아니었습니다.**
...
- **request를 넘거나 limit에 다가선 파드가 아직 있습니다** — 노드 메모리 8GB × 3 안에서는 전부에게 여유를 줄 수 없고, 자원이 큰 클러스터로 옮길 때 이 화면으로 잰 값으로 다시 잡습니다
