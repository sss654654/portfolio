...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| 지표 저장소 | **Mimir distributed** — ingester만 3대, 노드당 1 | 최근 2시간은 MinIO에 없고 ingester 메모리에만 있음 — 죽으면 그 구간이 빔. **알림 다섯과 판 전부가 지표 위에 서 있어 여기만은 구멍을 못 냄** — 셋이 같은 값을 들어 한 대가 죽어도 남게 함 |
| 로그·트레이스 | **Loki·Tempo는 단일** | 위험은 같지만 무게가 다름 — 로그는 실패·경합만 남기고(초당 16줄), 트레이스는 보존 24시간의 조사 도구. **복제에 드는 RAM은 지표에만 지불**하고 이쪽은 전용 WAL 디스크로 재시작만 복구 — 노드가 통째로 나갈 때의 유실은 감수 |
| 저장 몸통 | **클러스터 안 MinIO** — S3 호환 | 셋 다 원본은 여기 두고 로컬엔 WAL만 남김. 전용 계정이 세 버킷만 읽고 씀 — 루트 자격은 어느 앱에도 안 나감 |
| Tempo 보유 한도 | **5배로** — 동시 보유 1만 → 5만 | 트레이스를 전 요청 남기기로 하자(표본 1.0) queue 폴링이 초당 수천 건이라, 동시에 열려 있는 트레이스가 기본 한도를 넘어 넘친 만큼 버려짐. 버려진 구간은 나중에 열어 볼 트레이스가 없음 — 한도와 함께 메모리 상한도 3배로 |
| 알림에 패널 스크린샷 추가 | **Grafana Image Renderer** — 알림마다 그 지표의 패널을 그려 첨부 | 알림은 Discord로 옴 — 숫자만 오면 무슨 일인지, 얼마나 급한지 애매함. 그 지표 패널의 캡처가 같이 오면 언제부터 얼마나 올랐는지가 바로 읽힘. 렌더러가 죽어도 알림은 텍스트로 계속 감 |
{:.hl-dec}
...
| 판을 열면 패널들이 시간 초과 — 조회 파드가 **42회** 재시작 | 계기판에 넣은 패널 쿼리 하나가 **전 시리즈를 훑고** 있었음. 판을 열어 두면 30초마다 다시 도는 상시 부하가 되어, 조회 파드가 상한 256Mi를 그때마다 넘음 | 쿼리를 가벼운 것으로 바꾸고 상한 512Mi. 재시작 0 — 계기판을 세우는 일 자체가 저장소의 부하라는 걸 여기서 겪음 |
{:.hl-tbl}
