...
| 저장 몸통 | **클러스터 안 MinIO** — S3 호환 | 셋 다 원본은 여기, 로컬엔 WAL만. 전용 계정이 세 버킷만 읽고 쓰는 구성 — 루트 자격은 앱에 미노출 |
| Tempo 보유 한도 | **`max_traces_per_user` 5배** — 동시에 열려 있는 트레이스 1만 → 5만 | 전 요청을 남기기로 하자 폴링이 초당 수천 건이라 기본 한도를 넘음 — 넘친 트레이스는 버려져 열어 볼 수 없음 |
...
| 유휴인데 CPU 패키지 **92°C** — 예고 없이 꺼진 적이 있는데 그때 온도 기록이 없음 | 수집 경로 자체가 부재 — 온도·전원은 물리 호스트에만 있는 지표라 VM 안에서는 어느 경로로도 수집 불가 | 호스트에 node-exporter를 얹어 변수 하나씩 — 쿨러 · 덮개 상시 열기 · 거버너 powersave로 **66°C**. 이 실측이 알림 임계(90°C)의 근거 |
