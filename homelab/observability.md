...
| Tempo 보유 한도 | **5배로** — 동시 보유 1만 → 5만 | 트레이스를 전 요청 남기기로 하자(표본 1.0) queue 폴링이 초당 수천 건이라, 동시에 열려 있는 트레이스가 기본 한도를 넘어 넘친 만큼 버려짐. 버려진 구간은 그래프의 점을 눌러도 빈 화면 — 한도와 함께 메모리 상한도 3배로 |
| 알림에 패널 그림 | **렌더러를 따로 둠** — 알림마다 그 지표의 패널을 그려 첨부 | 폰에 숫자만 오면 얼마나 급한지 못 가름 — 언제부터 올랐는지는 곡선이 답함. 그리는 건 별도 파드(헤드리스 브라우저)가 하고, 그 파드가 죽어도 알림은 텍스트로 계속 감 |
{:.hl-dec}
...
| 건너감 | 실제로 하는 일 |
|---|---|
| 지표 → 트레이스 | p99 그래프의 **점**(exemplar)을 누르면 그 봉우리를 만든 요청 하나가 열림 |
