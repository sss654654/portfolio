...
title: CI/CD
description: >
  GitLab 파이프라인 1개가 이미지를 만들고, 노트북 ArgoCD 허브 1개가 dev · stg 두 클러스터에 배포합니다
permalink: /homelab/cicd/
---

<p class="hl-back" markdown="0"><a href="/homelab/">← 홈랩</a></p>

GitLab 파이프라인 1개 · ArgoCD 허브 1개로 dev · stg 두 클러스터에 배포합니다.
dev는 머지 시 자동, stg는 수동 승격 job 실행 시 **같은 이미지**가 배포되고, 환경 차이는 폴더의 값뿐입니다.
{:.lead}
...
| 항목 | 선택 | 이유 |
|---|---|---|
| Git 서버 위치 | **클러스터 밖 데스크탑** — 러너 · 레지스트리 포함 | GitHub은 사설망 ArgoCD 호출 불가 · 클러스터 안이면 동반 중단 · 빌드 I/O가 부하 측정에 간섭 |
| CI · CD | **분리** — 파이프라인은 이미지까지, 배포는 ArgoCD | 파이프라인이 배포하면 클러스터 전권 자격이 외부에 필요 — 분리하면 클러스터 안에만 존재 |
| 브랜치 · 환경 | **trunk 하나(`main`)** · 환경은 `envs/<환경>/` 폴더 | 브랜치를 환경으로 쓰면 환경 축이 둘(브랜치 · 폴더) · 승격이 태그 커밋 한 줄이라 이력 · 롤백 단순 |
| 승격 | **1회 빌드 · 같은 이미지를 ECR로** · 게이트는 수동 job `publish-ecr` | 부하 비교에 두 환경 이미지 동일 필요 — 환경별 빌드 시 코드 차이와 이미지 차이 구분 불가 · 수동 실행 = 승격 결정, 변수 게이트를 더하면 이중 게이트 |
| 원격 클러스터 | **집 허브가 EKS를 클러스터 이름으로 배포** | EKS에서 사설망 GitLab 접근 불가 · 주소 대신 이름 지정 — 켜는 날 주소 치환 20곳 제거 |
| 취약점 게이트(scan) | **수정판 있는 취약점만** 머지 차단 | 패치 없는 CVE 수십 개 — 차단해도 올릴 수정판이 없어 파이프라인만 중단 |
| 배포 권한 · 자격 | **AppProject**로 단위별 제한 · dev **SealedSecret** · stg **Secrets Manager** | ArgoCD는 관리자 권한 — 제한이 없으면 Application 하나로 모든 자원 생성 가능 · 봉인은 클러스터 개인키에 묶여 stg로 이전 불가 |
{:.hl-dec}
...
- **파이프라인 1개 · 허브 1개로 두 클러스터 배포** — dev 머지 후 **3초** 반영 · stg 코드 → 브라우저 **19분 41초**(기계 구간 1분 54초)
...
- **자동화 토큰 3개가 같은 날 만료** — 이미지 수신 · 태그 되쓰기 · 배포 정의 읽기가 동시 중단, 사전 알림 없음
