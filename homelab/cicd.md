...
| 정한 것 | 그렇게 한 이유 |
|---|---|
| Git 서버를 **클러스터 밖 데스크탑**에 | GitHub은 인터넷에 있어 사설망 안의 ArgoCD를 부를 수 없음. 그렇다고 클러스터 안에 세우면 클러스터가 무너질 때 다시 세울 근거도 함께 사라짐 |
| 러너도 **같은 데스크탑**에 | 노드 셋이 USB SSD 한 장을 나눠 씀. 빌드가 그 디스크를 쓰면 나중에 부하를 걸어 잰 숫자가 흔들림 |
| 빌드를 dind 대신 **도커 소켓**으로 | dind는 job마다 데몬을 새로 띄워 메모리가 더 들고 레이어 캐시가 매번 사라짐. 대가는 소켓을 쥔 쪽이 그 기계의 관리자가 되는 것 |
| 검사 다섯 중 **이미지 취약점만 예외** — 수정판이 나온 것만 막음 | 베이스 이미지를 훑으면 아직 패치가 없는 것이 수십 개 나옴. 고칠 수 없는 것으로 막으면 파이프라인이 늘 빨간불이 되고, 결국 게이트를 끄게 됨 |
| 기계에는 내 계정 토큰 대신 **저장소 전용 토큰** | 내 토큰은 내가 닿는 모든 프로젝트에 통함. 저장소 전용은 정해진 일 하나만 되고, 새면 그것만 폐기하면 됨 |
| 태그를 git에 되쓰는 일을 **클러스터 안 도구**에 | 파이프라인이 되쓰면 러너가 배포 정의 저장소의 쓰기 자격을 들어야 함. 러너는 위에서 소켓을 쥔 자리라 자격을 더 얹지 않음 |
{:.hl-why}
...
- **`cgv-infra` 쪽은 읽기와 쓰기가 한 자격에 합쳐져 있습니다** — image-updater에 전용 쓰기 자격을 주는 경로가 https·ssh 저장소만 받는데 이 GitLab은 평문 http라, ArgoCD 저장소 자격에 쓰기 토큰을 넣어 우회했습니다. TLS를 붙이는 날 되돌릴 항목입니다
