...
| 배포 권한 | **AppProject**로 App마다 읽을 저장소·쓸 네임스페이스·만들 리소스를 제한 | ArgoCD는 클러스터 관리자 권한으로 돎. 제한이 없으면 App 하나가 잘못돼도 클러스터에 뭐든 만들 수 있음 — 봉인된 자격을 배달하는 App은 `SealedSecret` 하나만 만들 수 있음 |
{:.hl-dec}
