...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| Git 서버 자리 | **클러스터 밖 데스크탑** — 러너도 같이 | GitHub은 사설망 안 ArgoCD를 호출 불가 · 안에 두면 함께 무너지고 빌드 I/O가 부하 실측을 교란 |
| 파이프라인 단위 | **바뀐 서비스의 job만 생성** — `rules: changes` | 한 저장소에 서비스 셋 — 조건이 없으면 한 줄만 고쳐도 전부 실행 · 안 바뀐 job은 스킵이 아니라 미생성 |
...
| 자격 보관 | **SealedSecret** — 봉인한 채 git에 커밋 | 자격만 손으로 넣으면 클러스터 재구축 때 재현 불가 — 봉인은 클러스터 안 개인키로만 복호 |
{:.hl-dec}
