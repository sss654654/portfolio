...
| 하루 열 번씩 GitLab·브라우저·편집기가 번갈아 죽음. 메모리 부족으로 보고 **램을 주문** | 물리는 **6.6GB 여유** — 천장은 Windows 커밋 한도 **17.9GB**, 페이지파일 2GB 고정이 그 상한 | 페이지파일을 12GB로. 한도 **28GB**. 램은 한 장도 안 바꾸고 **주문은 취소** |
{:.hl-tbl}
...
- **자동화를 잇는 토큰 셋이 같은 날 만료됩니다** — 이미지 받기·태그 되쓰기·배포 정의 읽기가 함께 멈추는데, 만료를 미리 알려 줄 장치가 없습니다
- **환경이 dev 하나입니다** — stg·prd는 자원이 큰 서버에 설 다음 자리라 배포 정의에 골격만 있고, 그때 검증된 이미지를 올리는 승격 절차부터 필요합니다
