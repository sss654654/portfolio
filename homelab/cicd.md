...
| 빌드 실행 | **도커 소켓** — dind 아님 | dind는 job마다 데몬을 새로 띄워 메모리가 더 들고 레이어 캐시가 매번 사라짐. 대가는 job이 그 소켓으로 데스크탑 도커 전체를 부를 수 있는 것 |
| 게이트 | **수정판이 나온 취약점만** 막음 | 베이스 이미지에 패치가 안 나온 취약점이 수십 개 있음. 그것까지 막으면 파이프라인이 늘 빨간불이라 결국 게이트를 끄게 됨 |
| 배포 방식 | 클러스터가 **당겨간다** — 밀지 않음 | 미는 쪽이면 파이프라인이 `kubectl`을 쳐야 하고, 그러려면 파이프라인이 클러스터 전권 자격을 듦. 당겨가면 그 자격이 클러스터 안에만 있음 |
{:.hl-dec}
...
| 한 판에 **10분 15초** | 판마다 같은 의존성과 취약점 DB를 다시 받음. `lint`는 job 컨테이너 안에서, `build`는 소켓 너머 데스크탑 도커가 받아 **캐시가 두 곳으로 갈림** | 러너 볼륨과 Dockerfile 캐시 마운트로 **양쪽 다** 남김. 열네 판째 **1분 54초** |
