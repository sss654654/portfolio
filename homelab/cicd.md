...
|---|---|---|
| 브랜치 | **`main` 하나** — 클러스터가 하나라 브랜치로 나눌 대상이 없음. 환경은 `environments/{dev,stg,prd}` 디렉터리가 가름 | **`dev` 기본 · `main`** — 브랜치가 이미지 태그의 앞부분이라, stg·prd가 생기면 태그에서 갈림 |
| 파이프라인 | 없음 | **다섯 단 13 job.** 데스크탑 러너가 돌리고, 통과해야 머지 버튼이 열림 |
...
| 정한 것 | 그렇게 한 이유 |
|---|---|
| Git 서버를 **클러스터 밖 데스크탑**에 | GitHub은 인터넷에 있어 사설망 안의 ArgoCD를 부를 수 없음 — ArgoCD 기본값인 폴링(최대 3분)만 남음. 그렇다고 클러스터 안에 세우면 클러스터를 다시 세울 근거가 그 클러스터와 함께 무너짐 |
| 두 저장소 모두 직접 push 차단 — **작업 브랜치를 따서 MR로** | 직접 push를 열어 두면 아래 게이트를 전부 우회할 수 있음. 무료판은 승인을 머지 조건으로 못 걸어, 파이프라인이 있는 `cgv-onprem`에는 **통과해야 머지**를 걸었음 |
| 검사 다섯 중 **이미지 취약점만 예외** — 수정판이 나온 것만 막음 | 베이스 이미지를 훑으면 아직 패치가 없는 것이 수십 개 나옴. 고칠 수 없는 것으로 막으면 파이프라인이 늘 빨간불이 되고, 결국 게이트를 끄게 됨 |
| 통과한 이미지에 **커밋마다 다른 이름** — `dev-15-17dcc495` | 이름을 고정하면 매니페스트가 안 변해 ArgoCD가 할 일이 없음. 파드를 갈려면 밖에서 클러스터에 명령해야 하는데, 그 순간 클러스터 자격이 밖에 있어야 함 |
| 그 이름을 git에 옮기는 쪽을 **클러스터 안 도구**로 | 파이프라인이 옮기려면 러너가 배포 정의 저장소의 쓰기 자격을 들어야 함. 러너는 도커 소켓까지 쥔 자리라 거기에 자격을 하나라도 덜 둠 |
...
| 파이프라인 한 판이 **10분 15초** | 판마다 의존성과 취약점 DB를 다시 받는데, **받는 자리가 둘**이었음 — lint는 job 컨테이너 안에서, build는 소켓 너머 데스크탑 데몬에서. 러너 볼륨은 데몬 쪽에 안 닿음 | 러너 볼륨과 Dockerfile 캐시 마운트, **두 층으로** 나눠 붙임. 열네 판째 **1분 54초** |
