...
GitHub은 인터넷에 있어 사설망 안의 ArgoCD를 부를 수 없습니다. Git 서버를 같은 망에, 단 정본이 대상과 함께 무너지지 않도록 **클러스터 밖**에 세웠습니다.
...
`scan`만 조건을 붙인 것은, 손쓸 수 없는 것으로 막으면 결국 게이트를 끄게 되기 때문입니다. 처음 켠 판에서 **72건**이 나왔고 원인이 서비스마다 달랐습니다.
...
셋 다 **고정해 둔 것이 낡은 것**입니다. 코드가 안 바뀌어 아무 신호가 없었고, 지금은 0건입니다.
