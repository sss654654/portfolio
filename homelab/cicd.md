...
|---|---|---|
| 브랜치 | **`main` 하나** — ArgoCD가 보는 브랜치가 곧 배포 상태. 환경은 `environments/` 디렉터리로 가름 | **`dev` 기본 · `main`** — 이미지 태그 앞부분이 브랜치 이름(`dev-15-…`). stg·prd가 생기면 여기서 갈림 |
...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| Git 서버 자리 | **클러스터 밖 데스크탑** — 러너도 같이 | GitHub은 사설망 안 ArgoCD를 못 부름. 클러스터 안에 두면 함께 무너지고, 러너 빌드 I/O는 부하 실측을 흔듦 |
| 파이프라인 단위 | **바뀐 서비스의 job만 생성** — `rules: changes` | 서비스 셋이 한 저장소라 조건이 없으면 한 줄만 고쳐도 전부 돎. 안 바뀐 것은 건너뛰는 게 아니라 아예 안 생김 |
| 취약점 게이트(scan) | **수정판이 나온 취약점만** 머지 차단 | 패치가 안 나온 CVE가 수십 개 — 그것까지 막으면 늘 빨간불이라 게이트 자체가 무력해짐 |
| 배포 방식 | **CI와 CD를 분리** — 파이프라인은 이미지까지, 배포는 클러스터 안 ArgoCD가 | 파이프라인이 배포까지 하려면 전권 자격이 필요 — 갈라 두면 클러스터 안에만 남음 |
| 배포 권한 | **AppProject** — 단위마다 읽을 저장소·쓸 네임스페이스·만들 리소스 제한 | ArgoCD는 클러스터 관리자 권한으로 동작 — 제한이 없으면 `Application` 하나의 실수로 무엇이든 생김 |
{:.hl-dec}
...
| 증상 | 원인 | 조치 |
|---|---|---|
| 취약점 스캔을 처음 켜자 **72건** | 셋 다 **버전 고정 후 방치** — 간접 의존 2건 · Spring Boot 부모 37건 · 갱신 멈춘 nginx 33건 | 간접 의존 상향 · 부모 버전 교체 · 베이스 이미지 이동. **0건** |
| 파이프라인 한 판에 **10분 15초** | 판마다 의존성과 취약점 DB를 다시 받음 — `lint`와 `build`가 받는 자리가 달라 **캐시가 두 곳으로 갈림** | 러너 볼륨과 Dockerfile 캐시 마운트로 **양쪽 다** 남김. 열네 판째 **1분 54초** |
| 하루 열 번씩 GitLab·브라우저·편집기가 번갈아 죽음. 메모리 부족으로 보고 **램을 주문** | 물리는 **6.6GB 여유** — 상한은 Windows 커밋 한도 **17.9GB**, 페이지파일 2GB 고정에서 나온 값 | 페이지파일을 12GB로. 한도 **28GB**. 램은 한 장도 안 바꾸고 **주문은 취소** |
{:.hl-tbl}
