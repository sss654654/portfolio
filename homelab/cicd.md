...
| 하루 열 번씩 GitLab·브라우저·편집기가 멈춤 — 메모리 부족으로 보고 **램을 주문** | 물리는 **6.6GB 여유** — 한도는 Windows 커밋 **17.9GB** 로, 페이지파일 2GB 고정에서 나온 값 | 페이지파일 12GB → 한도 **28GB** · 램 주문 **취소** |
{:.hl-tbl}
...
- **환경이 dev 하나입니다** — stg·prd는 자원이 큰 서버에 설 다음 자리라 배포 정의에 골격만 있고, 검증된 이미지를 올리는 승격 절차부터 필요합니다
