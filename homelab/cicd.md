...
| 정한 것 | 그렇게 한 이유 |
|---|---|
| Git 서버와 러너를 **클러스터 밖 데스크탑**에 | GitHub은 인터넷에 있어 사설망 안의 ArgoCD를 못 부름. 그렇다고 클러스터 안에 세우면 클러스터와 함께 무너짐. 러너도 같이 둔 것은 빌드 I/O가 노드 디스크에 섞이면 부하 실측이 흔들려서 |
...
| 배포를 파이프라인이 **미는 대신**, 클러스터 안 ArgoCD가 git을 읽어 **당겨가게** | 미는 쪽이면 파이프라인이 `kubectl`을 쳐야 하고, 그러려면 파이프라인이 **클러스터 전권 자격**을 들어야 함. 당겨가면 그 자격을 드는 쪽이 클러스터 안에만 있음 |
{:.hl-why}
...
|  | 전 | 후 |
|---|---|---|
| Git 서버 | 인터넷의 GitHub | 데스크탑의 **GitLab CE** — 저장소·러너·레지스트리가 한자리 |
| 커밋에서 클러스터 반영 | 최대 3분 (폴링) | **3초** (webhook) |
| 파이프라인 한 판 | 첫 판 10분 15초 | 열네 판째 **1분 54초** |
| 이미지 취약점 | 게이트를 켜자 **72건** | **0건** — 고칠 수 있는 것 기준 |
| 앱 세 종 | `ImagePullBackOff` | **`Running`** |
{:.hl-two}
