...
GitLab을 세우고 저장소 둘을 GitHub에서 옮겨 왔습니다.
...
| 발급한 자격 | 저장소 자격 **하나** — ArgoCD가 읽고, image-updater가 그것으로 되씀 | deploy token **둘** (`read_registry`) — 노드용·봇용을 따로 |
...
| 새 이미지 이름을 배포 정의에 적는 일을 **클러스터 안의 image-updater**에 | 그 줄이 바뀌어야 ArgoCD가 파드를 새 이미지로 갈아 끼움. 파이프라인이 적으면 러너가 배포 정의 저장소의 쓰기 권한을 갖는데, 러너는 이미 데스크탑 도커 소켓을 쥐고 있어 권한을 더 주지 않음 |
{:.hl-why}
...
<svg viewBox="0 0 760 556" role="img" aria-label="왼쪽 열에서 cgv-onprem 이 러너 다섯 단을 지나 레지스트리에 이미지로 서고, 오른쪽에서 image-updater 가 cgv-infra 의 tag 줄을 바꿔 ArgoCD 가 읽는다. 번호가 커밋에서 파드까지의 순서">
  ...
  <rect class="hla-inner" x="300" y="444" width="190" height="60" rx="4"/>
  <image href="/assets/img/icons/argo.svg" x="312" y="456" width="18" height="18"/>
  <text class="hla-t" x="336" y="470">image-updater</text>
  <text class="hla-s2" x="312" y="492">새 태그를 git 에 되쓴다</text>
...
- **러너가 데스크탑의 도커 소켓을 물고 있습니다** — 그 소켓을 쥔 쪽은 사실상 그 기계의 관리자입니다
