...
| 파이프라인 | 없음 — 빌드할 것이 없음 | **다섯 단** — 바뀐 서비스의 job만 뜸. 데스크탑 러너가 돌리고, 통과해야 머지 버튼이 열림 |
...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| Git 서버 자리 | **클러스터 밖 데스크탑** — 러너도 같이 | GitHub은 인터넷에 있어 사설망 안 ArgoCD를 못 부르고, 클러스터 안에 세우면 클러스터와 함께 무너짐. 러너를 여기 둔 것은 빌드 I/O가 노드 디스크에 섞이면 부하 실측이 흔들려서 |
| 게이트 | **수정판이 나온 취약점만** 막음 | 베이스 이미지에 패치가 안 나온 취약점이 수십 개 있음. 그것까지 막으면 파이프라인이 늘 빨간불이라 결국 게이트를 끄게 됨 |
| 배포 방식 | **CI와 CD를 분리** — 파이프라인은 이미지까지, 배포는 클러스터 안 ArgoCD가 | 파이프라인이 배포까지 하려면 `kubectl`을 쳐야 하고, 그러려면 클러스터 전권 자격을 파이프라인이 듦. 갈라 두면 그 자격이 클러스터 안에만 있고, 파이프라인은 이미지만 만들면 됨 |
| 배포 권한 | **AppProject**로 배포 단위(`Application`)마다 읽을 저장소·쓸 네임스페이스·만들 리소스를 제한 | ArgoCD는 클러스터 관리자 권한으로 돎. 제한이 없으면 그중 하나가 잘못돼도 클러스터에 뭐든 만들 수 있음 — 봉인된 자격을 배달하는 쪽은 `SealedSecret` 하나만 만들 수 있음 |
{:.hl-dec}
...
| 증상 | 원인 | 조치 |
|---|---|---|
| 취약점 스캔을 처음 켜자 **72건** | 셋 다 **버전을 고정해 두고 오래 안 건드린 것** — `queue-go` 2건은 딸려 온 간접 의존, `booking` 37건은 Spring Boot 부모 버전, `frontend` 33건은 업스트림이 갱신을 멈춘 nginx 베이스 | 간접 의존 상향 · 부모 버전 교체 · 베이스 이미지 이동. **0건** |
| 파이프라인 한 판에 **10분 15초** | 판마다 같은 의존성과 취약점 DB를 다시 받음. `lint`는 job 컨테이너 안에서, `build`는 소켓 너머 데스크탑 도커가 받아 **캐시가 두 곳으로 갈림** | 러너 볼륨과 Dockerfile 캐시 마운트로 **양쪽 다** 남김. 열네 판째 **1분 54초** |
| 데스크탑에서 하루 열 번씩 뭔가가 죽음 — GitLab·브라우저·편집기가 번갈아. 메모리 부족으로 보고 **램을 주문** | 물리 메모리는 **6.6GB가 남아 있었고**, 죽은 것도 GitLab이 아니라 WSL을 켜는 명령이었음. 진짜 천장은 Windows가 약속할 수 있는 총량 **17.9GB** — 페이지파일을 2GB로 고정해 둔 옛 설정이 그 값을 정하고 있었음 | 페이지파일을 12GB로. 한도 **28GB**. 램은 한 장도 안 바꾸고 **주문은 취소** |
{:.hl-tbl}

## 결과

- Git 서버가 인터넷의 GitHub에서 **데스크탑의 GitLab CE**로 옮겨졌고, 러너와 레지스트리가 한자리에 섰습니다
- 두 저장소 모두 **머지가 곧 배포입니다** — 앱은 CI와 이미지를 거쳐, 배포 정의는 바로. 커밋 뒤에 `kubectl`을 칠 일이 없습니다
- 커밋에서 클러스터 반영까지 최대 3분 폴링이던 것이 **3초**가 됐습니다
- **앱 세 종이 처음으로 떴습니다** — 배선을 세운 이래 받아 올 이미지가 없어 멈춰 있던 파드들입니다

## 남은 것

- **자동화를 잇는 토큰 셋이 같은 날 만료됩니다** — 이미지 받기·태그 되쓰기·배포 정의 읽기가 그날 함께 멈추는데, 만료를 미리 알려 줄 장치가 아직 없습니다
- **이미지가 가는 환경이 dev뿐입니다** — 배포 정의의 stg·prd 디렉터리는 골격만 있고, 검증된 이미지를 그리로 올리는 승격 경로는 아직 없습니다
