...
- `cgv-onprem`은 **다섯 단 13 job**을 통과한 이미지만 레지스트리에 올립니다
- `cgv-infra`는 ArgoCD가 읽어 클러스터를 맞추고, 새 이미지 이름은 argocd-image-updater가 적습니다
- **앱 세 종이 처음으로 떴습니다** — 배선을 세운 이래 받아 올 이미지가 없어 멈춰 있던 파드들입니다

## 남은 것

- **빌드 job이 데스크탑 도커 전체에 닿을 수 있습니다** — 러너에 도커 소켓이 마운트돼 있어, 파이프라인이 오염되면 GitLab과 레지스트리가 있는 그 기계까지 노출됩니다
