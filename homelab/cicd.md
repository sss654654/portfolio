...
| 파이프라인 | 없음 — 빌드할 것이 없음 | **다섯 단** — 데스크탑 러너가 돌리고 통과해야 머지 버튼이 열림 |
...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| Git 서버 자리 | **클러스터 밖 데스크탑** — 러너도 같이 | GitHub은 사설망 안 ArgoCD를 못 부름 · 안에 두면 함께 무너지고 빌드 I/O가 부하 실측을 흔듦 |
| 파이프라인 단위 | **바뀐 서비스의 job만 생성** — `rules: changes` | 한 저장소에 서비스 셋이라 조건이 없으면 한 줄 고쳐도 전부 돎 · 안 바뀐 것은 건너뛰는 게 아니라 아예 안 생김 |
| 취약점 게이트(scan) | **수정판이 나온 취약점만** 머지 차단 | 패치가 없는 CVE가 수십 개 — 막아도 올릴 수정판이 없어 파이프라인만 멈춤 |
| 배포 방식 | **CI와 CD를 분리** — 파이프라인은 이미지까지, 배포는 클러스터 안 ArgoCD가 | 파이프라인이 배포까지 하려면 전권 자격이 밖에 있어야 함 — 갈라 두면 클러스터 안에만 남음 |
| 배포 권한 | **AppProject** — 단위마다 읽을 저장소·쓸 네임스페이스·만들 리소스 제한 | ArgoCD는 클러스터 관리자 권한으로 동작 — 제한이 없으면 `Application` 하나로 무엇이든 생김 |
| 자격 보관 | **SealedSecret** — 봉인한 채 git에 커밋 | 자격만 손으로 넣으면 클러스터를 다시 세울 때 재현이 끊김 — 봉인은 클러스터 안 개인키로만 풀림 |
{:.hl-dec}
...
| 파이프라인 한 판에 **10분 15초** | 판마다 의존성과 취약점 DB를 다시 받음 — 받는 자리가 `lint`·`build`로 갈려 **캐시가 두 곳** | 러너 볼륨과 Dockerfile 캐시로 양쪽 다 남김 → 열네 판째 **1분 54초** |
| 하루 열 번씩 GitLab·브라우저·편집기가 멈춤 — 메모리 부족으로 보고 **램을 주문** | 물리는 **6.6GB 여유** — 한도가 Windows 커밋 **17.9GB**, 페이지파일 2GB 고정에서 나온 값 | 페이지파일 12GB → 한도 **28GB** · 램 주문 **취소** |
{:.hl-tbl}
...
- 두 저장소 모두 **머지가 곧 배포입니다** — 커밋 뒤 `kubectl`을 칠 일이 없습니다
- 커밋에서 클러스터 반영까지 최대 3분 폴링이던 것이 **3초**가 됐습니다
- **저장소에 평문 자격이 없습니다** — 시크릿 **17종**이 봉인된 채 커밋돼 있고, 클러스터 안 개인키로만 풀립니다

## 남은 것

- **자동화를 잇는 토큰 셋이 같은 날 만료됩니다** — 이미지 받기·태그 되쓰기·배포 정의 읽기가 함께 멈추는데, 미리 알려 줄 장치가 없습니다
- **환경이 dev 하나입니다** — 배포 정의에 stg·prd 골격만 있고, 검증된 이미지를 올리는 승격 절차가 없습니다

## 쓴 것

GitLab CE · GitLab Runner · Docker · Trivy · gitleaks · SpotBugs · ArgoCD · argocd-image-updater · Sealed Secrets
{:.hl-more}
