...
|---|---|---|
| 브랜치 | **`main` 하나** — ArgoCD가 보는 브랜치가 곧 배포된 상태. 환경은 `environments/` 아래 디렉터리가 가름 | **`dev` 기본 · `main`** — 이미지 태그의 앞부분이 브랜치 이름(`dev-15-…`). stg·prd가 생기면 여기서 갈림 |
...
| 배포 권한 | **AppProject** — 배포 단위마다 읽을 저장소·쓸 네임스페이스·만들 리소스를 제한 | ArgoCD는 클러스터 관리자 권한으로 동작 — 제한이 없으면 `Application` 하나의 실수로 무엇이든 생성 가능 |
{:.hl-dec}
