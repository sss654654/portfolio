...
| 발급한 자격 | 저장소 자격 **하나** — ArgoCD가 매니페스트를 읽고, argocd-image-updater가 태그를 적는 데 공용 | deploy token **둘** (`read_registry`) — 노드가 이미지 받을 것과 봇이 태그 볼 것을 따로 |
...
| 취약점 게이트(scan) | **수정판이 나온 취약점만** 머지 차단 | 이미지 스캔에 잡히는 CVE 중 패치가 안 나온 것이 수십 개 — 그것까지 막으면 늘 빨간불이라 결국 게이트 자체가 무력화 |
| 배포 방식 | **CI와 CD를 분리** — 파이프라인은 이미지까지, 배포는 클러스터 안 ArgoCD가 | 파이프라인이 배포까지 하려면 클러스터 전권 자격이 필요 — 갈라 두면 그 자격의 보관처가 클러스터 안으로 한정 |
| 배포 권한 | **AppProject**로 배포 단위(`Application`)마다 읽을 저장소·쓸 네임스페이스·만들 리소스를 제한 | ArgoCD는 클러스터 관리자 권한으로 동작 — 제한이 없으면 `Application` 하나의 실수로 무엇이든 생성 가능 |
{:.hl-dec}
...
| 데스크탑에서 하루 열 번씩 뭔가가 죽음 — GitLab·브라우저·편집기가 번갈아. 메모리 부족으로 보고 **램을 주문** | 물리 메모리는 **6.6GB 여유** — 진짜 천장은 Windows가 약속할 수 있는 총량(커밋 한도) **17.9GB**였고, 페이지파일 2GB 고정이라는 옛 설정이 그 값의 상한 | 페이지파일을 12GB로. 한도 **28GB**. 램은 한 장도 안 바꾸고 **주문은 취소** |
{:.hl-tbl}
...
- **환경이 dev 하나입니다** — stg·prd는 자원이 큰 서버·클라우드에 설 다음 자리로 배포 정의에 골격만 있고, 환경이 서면 검증된 이미지를 올리는 승격 절차부터 필요합니다
