...
- **booking은 한 대입니다** — 마이그레이션 도구가 없어 Hibernate가 스키마를 만들고, 여러 대가 같이 뜨면 서로 다른 DDL을 돌립니다. 늘리려면 도구 도입이 먼저입니다
- **입장과 회수가 서로 다른 토픽이라 처리 순서가 보장되지 않습니다** — 회수가 먼저 처리되면 뒤늦은 입장이 이미 자리를 잃은 사람의 인증을 되살립니다. 성립 조건(입장 소비가 세션 타임아웃보다 밀림)과 닫는 방법(이벤트 모델 변경)은 코드 주석에 적어 뒀습니다
