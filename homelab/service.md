...
| 첫 행을 고쳤는데 1만 명 판에서 인증이 다시 **19.86초** | 좌석 현황판이 **대기열 게이트 밖** — 아직 못 들어온 사람도 3초마다 부름. 요청 수가 대기 인원에 비례해 booking에 쌓이고, 같은 프로세스의 Kafka 소비 스레드가 CPU를 못 받음 | 응답에 `Cache-Control` 5초를 붙여 엣지가 대신 답하게 함. origin 도착 48,500 → **26건** |
| 오픈 3초 만에 booking이 **5,000건**을 받고 readiness 실패 — 파드가 트래픽에서 빠짐 | 영화 목록도 게이트 밖 — 대기자가 아니라 **접속한 전원**이 부름. 앞 행과 같은 자리인데 경로만 다름 | frontend nginx에 응답 캐시 10초 + `proxy_cache_lock` — 캐시가 비어도 origin으로 나가는 요청은 하나. 10초에 **2건** |
{:.hl-tbl}
