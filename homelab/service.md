...
| 관측 | **코드에 계측을 심음** | **기본 지표만으론 "어디서"가 안 보임**<br>파드 지표는 양만 답함. 요청 수·지연은 메트릭, 사건은 로그, 구간별 흐름은 트레이스 — 뒤의 둘을 trace_id로 연결 |
{:.hl-dec}
...
- **booking과 MySQL이 한 대씩입니다** — booking을 늘리려면 스키마 마이그레이션 도구가 먼저입니다. 지금은 Hibernate가 기동할 때 스키마를 만들어, 두 대가 같이 뜨면 같은 DB에 DDL을 겹쳐 돌립니다. 그 뒤에는 MySQL로 쓰기가 몰리므로 복제본과 읽기·쓰기 분리가 숙제입니다
