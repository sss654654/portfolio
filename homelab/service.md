...
| 증상 | 원인 | 조치 |
|---|---|---|
| 오픈 순간 입장 인증이 **9.94초**(p99) 뒤에야 적힘 — 그동안 그 사람은 좌석 화면에서 403 | Kafka 리스너가 레코드 한 건마다 불리고 그 안에서 Redis를 한 번 왕복 — 소비 상한이 **초당 28건**으로 고정됨. 오픈에 몰린 수백 건은 줄 길이만큼 밀림 | 배치 리스너로 바꾸고 Redis 쓰기를 파이프라인 한 왕복으로 묶음 — 호출도 왕복도 묶음당 1회 |
| 좌석 현황판 캐시가 만료되는 순간 booking 전체가 5xx **초당 467건** — 현황판과 무관한 요청까지 같이 실패 | 캐시가 비면 대기 중인 전원이 같은 질의를 동시에 던짐 — 각자 커넥션을 잡아 **풀 10개가 바닥남.** 트레이스로 가르니 질의 자체는 **10.7ms**, 커넥션을 기다린 시간이 **5.11초** | 만료되면 한 명만 계산하고 나머지는 직전 값을 받게 함(single-flight). p99 9.38초 → **41ms** |
| 첫 행을 고쳤는데 1만 명 판에서 인증이 다시 **19.86초** | 좌석 현황판이 **대기열 게이트 밖** — 아직 못 들어온 사람도 3초마다 호출. 요청 수가 대기 인원에 비례해 booking에 쌓이고, 같은 프로세스의 Kafka 소비 스레드는 CPU 경합에서 뒤로 밀림 | 응답에 `Cache-Control` 5초를 붙여 엣지가 대신 답하게 함. origin 도착 48,500 → **26건** |
| 오픈 3초 만에 booking이 **5,000건**을 받고 readiness 실패 — 파드가 트래픽에서 제외 | 영화 목록도 게이트 밖 — 대기자가 아니라 **접속한 전원**이 부름. 앞 행과 같은 자리인데 경로만 다름 | frontend nginx에 응답 캐시 10초 + `proxy_cache_lock` — 캐시가 비어도 origin으로 나가는 요청은 하나. 10초에 **2건** |
{:.hl-tbl}
