...
부하를 걸자 드러난 코드 결함들입니다 — 스펙(정원·자원) 값의 변경은 [부하테스트](/homelab/capacity/)에 있고, 여기는 코드가 바뀐 것만 적습니다.

| 증상 | 원인 | 조치 |
|---|---|---|
| 오픈 순간 입장 인증 등록이 밀림 — p99 **9.94초** | booking이 admissions를 레코드 한 건씩 소비 — 건마다 Redis 왕복, 상한 초당 28건 | **Kafka 배치 리스너 + Redis 파이프라인**. 전원 5초 안 |
| 캐시가 만료되는 순간 앱 전체 무응답 — 5xx 초당 467건, DB 풀이 전부 잠김 | 좌석 현황판의 1초 캐시가 만료되면 대기 전원이 동시에 재계산 — 트레이스로 가르니 질의는 10.7ms, 커넥션 대기가 5.11초 | **single-flight** — 만료 시 한 명만 계산하고 나머지는 직전 값. p99 9.38초 → **41ms** |
| 10,000명 판에서 인증 등록 19.86초 — booking CPU가 폴링에 잠김 | 좌석 현황판이 대기열 게이트 밖 — 대기 전원이 3초마다 불러 요청이 인원에 비례하고, Kafka 리스너가 CPU를 못 받음 | 응답에 **Cache-Control 5초**를 붙여 엣지가 받게 함. origin 도착 48,500 → **26건** |
| 오픈 3초에 booking이 5,000건을 받고 readiness 실패 | 영화 목록(`/api/movies`)이 게이트 밖 — 방문자 전원이 부름 | **frontend nginx 응답 캐시 10초 + proxy_cache_lock**. 10초에 **2건** |
{:.hl-tbl}
