...
올린 앱은 티케팅 서비스입니다 — 열리는 순간 전원이 한꺼번에 몰리는데 좌석은 4,000석뿐입니다.
2024년 한국시리즈 극장 생중계 예매에서 대기 인원이 16만 명까지 간 것을 모델로,
받아낼 규모를 **10만 명**으로 잡았습니다.

## 서비스 구조

**줄 세우는 쪽(queue)**과 **표를 파는 쪽(booking)**으로 나눴습니다.
사이는 **Kafka가 비동기로 연결**하고, 둘을 묶는 값은 `active` —
정원(동시에 들여보낼 인원)입니다.
...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| queue | **Go · 4대 고정** | **요청이 짧고 많음 — 대기열**<br>goroutine(수 KB)이 하나씩 맡아 동시 처리 비용이 작고, 뜨는 즉시 제 속도(빌드 때 기계어). 오픈은 시각을 아는 부하라 4대를 미리 띄움 — HPA는 봉우리보다 늦게 반응 |
| 순번·현황 | **Redis 폴링** | **홈·대기 화면이 주기마다 묻는 구조**<br>줄·정원·현황이 전부 Redis에 있어 왕복 1-2번, 1ms에 끝남 — 부하는 횟수(CPU)이고 어느 파드가 받아도 같은 답 |
| booking | **Java Spring · 한 대** | **요청이 길고 적음 — 입장객**<br>MySQL을 기다리는 동안 메모리를 쥐고, 수는 정원으로 묶임. 돈·좌석의 전부-성공/전부-롤백을 `@Transactional` 하나로 걺 — 대가는 기동부터 드는 수백 MiB |
| 서비스 간 통신 | **Kafka 비동기** — 서로 직접 호출 없음 | **한쪽이 죽어도 다른 쪽은 계속**<br>동기 호출이면 booking이 죽는 순간 queue도 정지. 발행하고 끝 — 못 받은 것은 토픽에 남아 나중에 따라잡기 |
| 관측 | **코드에 계측을 심음** — 메트릭 · 로그 · 트레이스 | **기본 지표만으론 "어디서"가 안 보임**<br>파드 CPU·메모리는 양만 답함. 요청 수·지연은 메트릭, 사건은 로그, 한 건의 구간별 흐름은 트레이스 — 뒤의 둘에 같은 trace_id를 실어 서로 연결 |
{:.hl-dec}

## 결과

- **서비스 둘과 프론트가 클러스터에서 돕니다** — queue 4대 · booking 1대 · frontend 2대. 배포는 MR 머지가 전부입니다
- **세 신호가 코드에 심어졌습니다** — 요청 수·지연(메트릭) · 사건(로그) · 흐름(트레이스). 셋이 trace_id로 이어져, 지표에서 이상을 보면 그 요청의 트레이스와 로그까지 내려갑니다
- **정원·커넥션 풀·메모리 상한은 아직 가정값입니다** — 이 계측 위에서 부하를 걸어 실측으로 바꾸는 것이 [부하테스트](/homelab/capacity/)입니다

## 남은 것

- **Redis 한 대가 읽기와 쓰기를 다 받습니다** — Sentinel은 장애 시 승격이지 부하 분산이 아니고, 명령 처리가 단일 스레드라 코어를 더 줘도 하나만 씁니다. 대기 인원이 늘면 여기가 먼저 찹니다 — 순번 조회를 복제본으로 돌리는 것이 첫 수순이고, 모자라면 키를 나눠 여러 대에 두는 샤딩입니다
- **booking과 MySQL이 각각 한 대입니다** — booking을 늘리려면 스키마 마이그레이션 도구가 먼저입니다. 지금은 Hibernate가 기동할 때 스키마를 만들어, 두 대가 같이 뜨면 같은 DB에 DDL을 겹쳐 돌립니다. 그 뒤에는 MySQL로 쓰기가 몰리므로 복제본과 읽기·쓰기 분리가 그다음 숙제입니다
