...
| 순번·현황 | **Redis에 묻는다(폴링)** | **홈·대기 화면이 주기마다 묻는 구조**<br>줄·정원·현황이 전부 Redis에 있어 왕복 1-2번, 1ms에 끝남 — 부하는 횟수(CPU)이고 어느 파드가 받아도 같은 답 |
...
| 관측 | **코드에 계측을 심음** — 메트릭 · 로그 · 트레이스 | **기본 지표만으론 "어디서"가 안 보임**<br>파드 CPU·메모리는 양만 답함. 요청 수·지연은 메트릭, 한 건의 구간별 흐름은 트레이스, 사건은 로그 — 뒤의 둘에 같은 trace_id를 실어 서로 연결 |
{:.hl-dec}
