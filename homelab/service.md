...
| 증상 | 원인 | 조치 |
|---|---|---|
| **booking이 입장 인증을 늦게 적음**<br>승격 발행부터 인증 생성까지 p99 **9.94초**. 그 사이 좌석 요청은 게이트에서 403 | **리스너가 레코드 한 건마다 호출됨**<br>한 건마다 Redis 왕복 1회. 소비 처리량 상한 **초당 28건** | **배치 리스너 + Redis 파이프라인**<br>호출과 왕복이 묶음당 1회 |
| **배치로 바꾼 뒤 인증 지연이 19.86초**<br>소비 쪽을 고쳤는데 더 나빠짐 | **좌석 현황판 API가 입장 게이트 밖**<br>입장 못 한 대기자도 3초마다 호출 — 요청 수가 대기 인원에 비례. 같은 프로세스의 Kafka 소비 스레드가 CPU를 못 받음 | **응답에 `Cache-Control` 5초**<br>엣지가 대신 반환. origin 도착 48,500 → **26건** |
| **좌석 현황판 캐시가 만료될 때마다 booking 전체가 5xx**<br>초당 **467건**. 무관한 좌석·예매 요청까지 실패 | **만료 순간의 요청이 전부 각자 재계산**<br>각자 커넥션을 점유해 **풀 10개** 고갈. 트레이스로 가르면 질의 **10.7ms** · 커넥션 대기 **5.11초** | **single-flight**<br>재계산은 한 요청만, 나머지는 직전 값. p99 9.38초 → **41ms** |
{:.hl-tbl}
