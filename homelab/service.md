...
부하를 걸자 드러난 코드 결함 넷입니다. 자원·정원 값의 변경은 여기 없습니다 — 코드가 바뀐 것만 적습니다.

| 증상 | 원인 | 조치 |
|---|---|---|
| **승격은 나갔는데 인증이 안 적힙니다** — 오픈 순간 p99 9.94초. 그동안 그 사람은 좌석 화면에서 403 | Kafka 리스너가 **레코드 한 건마다 한 번** 불리고, 그 안에서 Redis에 한 번 왕복합니다. 소비 상한이 초당 28건이라 오픈에 몰린 수백 건이 줄을 섭니다 | 리스너를 **배치**로 바꾸고 Redis 쓰기를 **파이프라인 한 왕복**으로 묶습니다 — 호출도 왕복도 묶음당 1회가 됩니다 |
| **캐시가 만료되는 순간 booking 전체가 5xx** — 초당 467건. 현황판과 무관한 요청까지 실패 | 좌석 현황판의 1초 캐시가 만료되면 **대기 전원이 동시에 재계산**에 들어갑니다. 각자 DB 커넥션을 잡아 풀 10개가 바닥납니다 — 트레이스로 가르니 질의는 10.7ms인데 **커넥션 대기가 5.11초**였습니다 | **single-flight** — 만료 시 한 명만 계산하고 나머지는 직전 값을 받습니다. p99 9.38초 → **41ms** |
| **인증 등록이 다시 느려집니다** — 10,000명에서 19.86초. 첫 행을 고친 뒤인데도 | 좌석 현황판이 **대기열 게이트 밖**이라 대기 중인 사람도 3초마다 부릅니다. 요청이 대기 인원에 비례해 쌓이고, 같은 프로세스의 **Kafka 소비 스레드가 CPU를 못 받습니다** | 응답에 **`Cache-Control` 5초**를 붙여 엣지가 대신 답하게 합니다. origin 도착 48,500 → **26건** |
| **오픈 3초에 booking이 readiness 실패** — 5,000건을 받고 트래픽에서 빠짐 | 영화 목록(`/api/movies`)도 **게이트 밖**입니다 — 대기자가 아니라 **방문자 전원**이 부릅니다 | frontend **nginx 응답 캐시 10초 + `proxy_cache_lock`** — 동시에 캐시가 비어도 origin으로는 한 번만 갑니다. 10초에 **2건** |
{:.hl-tbl}
