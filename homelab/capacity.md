...
k6가 실제 여정 그대로 — **인원 1,000 → 10,000**(정원 2 고정) → **정원 2 → 500** →
공개 경로에서 **정원 1,000 · 인원 30,000**. 한 판에 한 변수만 바꿨습니다.
올린 값의 상한은 노드 스펙(8GB · 4vCPU × 3대)이고, 최종값은 전부 그 안에서 정해졌습니다.

| 스펙 | 가정 → 확정 | 부하가 드러낸 것 → 올린 뒤 |
|---|---|---|
| traefik | 2대×768Mi → **3대×2Gi** | 연결이 사람 수만큼 열림(사람당 132KiB) — 10,000명에서 limit 도달 → 40% |
| queue CPU | 500m → **1코어** | 스로틀 83% · Redis 풀 포기 초당 59건 → 둘 다 0 |
| booking 메모리 | 1Gi → **1,536Mi** (힙 768Mi 고정) | 힙+비힙 990Mi로 OOMKill 2회 → 워킹셋 47% |
| booking CPU | 1코어 → **2코어** | Kafka 소비 스레드 넷이 한 코어에 몰림 — 스로틀 99.7% → 32% |
| 커넥션 풀 | 10 → **30** | 대기 397건인데 MySQL은 CPU 12%로 유휴 → 대기 0 |
| 승격 | 100명/2초 → **25명/0.5초** | 묶음 100명이 인증 지연 4.97초를 만듦(초당 상한 50은 동일) → 0.99초 |
| 정원 | 2 → **1,000** | 자원이 아니라 좌석 4,000이 상한 — 더 올리면 100초에 소진돼 잴 구간이 없음 |
{:.hl-cmp}

- **10,000명 · 정원 1,000 판 통과** — 76만 요청(대부분 폴링) · 5xx 0 · 재시작 0 · 풀 대기 0 · 전원 완주
- **좌석 4,000이 440초에 매진** — 마지막 구매자의 대기 3분 7초
- **인원만 5배 올려도 booking 불변** — 줄이 길어져도 뒤는 밀리지 않았음
- **단위당 소모량도 실측됨** — 사람 1명·확정 1건이 쓰는 자원의 양. 목표를 넣으면 스펙이 계산됨: "좌석 4,000을 10분에"(확정 6.7건/초) → booking CPU 18% · MySQL 7%
