...
- **점진 부하** — 계단으로 올립니다. **인원 1,000 → 10,000** · **정원**(`MAX_SESSIONS` — queue가 동시에 입장시키는 인원) **2 → 1,000**
...
<div class="hl-shots" markdown="0" aria-label="queue 대시보드 — 화살표로 넘겨 봅니다">
  <figure class="hl-shot">
    ...
    → 이 판: <b>메모리 파드 최대 12%</b> · <b>연결 최대 327</b>.</figcaption>
  </figure>
  <figure class="hl-shot">
...
| 폴링 3초 | `position` **0.94초** · `events` **0.86초** · `board` **0.10초** — 전부 오픈 직후 봉우리 | 통과 |
...
- **30,000은 못 넘겼습니다** — 엣지 → traefik 연결이 10,000 판의 **458**에서 **22,866**으로 늘어, 그 메모리가 limit의 **90%**, 노드 여유가 **2.3GB에서 1.1GB**로 줄었습니다. 그러자 etcd 디스크 쓰기가 0.06초에서 **4초**가 되고 k3s가 재시작했습니다
- **연결이 왜 그만큼 늘었는지는 못 밝혔습니다** — 엣지가 방문자 연결을 대신 받는 구조인데, 30,000에서는 그렇게 되지 않았습니다
- **그 위 규모를 재려면 스펙 상향이 먼저입니다** — traefik 메모리·CPU limit과 노드 RAM. 8GB × 3대가 이 노트북의 상한이라 더는 홈랩 밖입니다
