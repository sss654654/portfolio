...
데스크탑 판으로 서비스는 선 안에 들어왔습니다. 남은 문제 둘은 서비스가 아니라
**재는 쪽**(생성기와 그 회선)에 있습니다.

| 남은 문제 | 왜 데스크탑에선 못 푸나 | 클라우드로 옮기면 |
|---|---|---|
| **인원 상한 10,000** | 1만 VU가 약 4코어 · 4GB를 써 WSL 메모리가 참 — 목표인 30,000명 부하는 못 만듦 | 30,000명을 만들려면 생성기가 클라우드로 가야 함 — 상한이 장비가 아니라 **인스턴스 스펙**이 됨 |
| **연결 실패 1.27%** | 실패 내용은 연결 리셋 · 타임아웃. 서버 · OPNsense · Cloudflare 설정 · k6 자원은 지표로 원인이 아님을 확인 — 남은 구간(공유기 · 엣지)은 지표가 없음 | 데스크탑 판은 나가는 요청도 엣지를 돌아 들어오는 요청도 **공유기 하나**를 지남. 클라우드 판은 방문자처럼 서비스로 **들어올 때만** 지남 — 옮긴 뒤에도 실패가 남으면 엣지, 없어지면 공유기 |
{:.hl-dec}

서비스는 이미 인터넷에 열려 있어, 밖에서 쏘면 방문자와 같은 경로로 잽니다.
