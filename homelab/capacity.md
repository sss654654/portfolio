...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| queue | **Go · 4대** — 늘려서 나누는 쪽 | **요청이 짧고 많음 — 대기열**<br>몇 명이 몰릴지 알 수 없는 자리. 요청 하나를 goroutine(수 KB)이 맡아 같은 메모리로 훨씬 많이 받고, 빌드 때 이미 기계어라 예열 없이 첫 요청부터 같은 속도 — 파드를 늘리면 다음 요청부터 바로 나뉨 |
...
재시작 0 · 풀 대기 0 · 전원 여정 완주. 인원만 5배 올린 판에서도 booking은 그대로였습니다 —
**줄이 아무리 길어도 뒤가 안 밀린다**는 설계가 실측으로 닫혔습니다.
그리고 값보다 오래 남는 산출물이 하나 더 있습니다:
