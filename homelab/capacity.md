...
<!-- 앞 카드들을 거쳐 온 사람에게 보여 줄 셋: 기준을 먼저 세웠다(SLO) → 통과했다(결과 + 화면) → 판마다 먼저 막힌 곳을 찾아 구조를 바꿨다(병목 표).
     병목 표의 조치가 클라우드 카드의 노드 구조로, 결과의 정원 비례가 서비스 카드의 정원으로 이어진다 -->

기준 사례 — 2024년 한국시리즈 극장 생중계 예매, 대기 16만 명.
SLO 5개를 테스트 전에 확정하고, 로비 → 줄서기 → 예매까지 실제 사용자 여정을 k6로 재현.
판마다 **먼저 막힌 곳**을 찾아 구조를 바꾸며 dev 1만 → stg 5만 명까지 통과.
{:.lead}
...
- **불변식** — 정원 초과 0 · 미소비 잔량 0 · 5xx 0 · 입장 후 403 0, 하나라도 어기면 그 판은 무효
- **판정** — 서버 지표(Mimir) 기준, 창은 오픈 −60초부터 +5분 · 판마다 좌석 4,000 · 대기 0에서 시작
...
- **병목 축은 파드당 동시 요청 수** — 사전 계산은 두 환경 모두 Redis를 지목했지만 두 번 모두 동시 연결이 먼저 한계. 조정 대상은 파드 수와 파드당 동시 요청 상한
