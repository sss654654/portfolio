...
| **30,000에서 k3s 컨트롤플레인 정지** | **엣지 → traefik TLS 연결이 41 → 22,866**<br>origin 응답이 3ms → 2.4초로 느려지자 엣지가 처리 중인 요청 수만큼 연결을 열었습니다(1만에서 458이었던 것은 origin이 빨라서). 연결마다 고루틴과 버퍼가 붙어 traefik 메모리가 limit 2Gi의 **90%**, 노드 여유 메모리가 2.3GB → **1.1GB**. 메모리가 마르자 etcd의 디스크 쓰기가 0.06 → **4초**로 늘었고, k3s가 스스로 재시작 | **미적용 — 갈래 둘**<br>traefik 스펙 상향(메모리 limit · 파드 수)은 3만을 받게 하지만 노드 8GB 안에서 상한이 있음. 동시 요청 상한(traefik `inFlightReq`)은 느려질 때 초과분을 즉시 429로 돌려 연결이 폭발하지 않게 함 — 무너지는 대신 빠르게 거절. 예측했던 Redis 단일 스레드 한계에는 닿지 못함 |
{:.hl-tbl}
