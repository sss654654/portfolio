...
같은 판을 반복하며 축 둘을 차례로 올렸습니다 — **인원**(줄에 서는 수 — queue의 부하)과
**정원**(동시에 예매하는 수 — booking의 부하). 합격선을 먼저 정하고 판마다 대조했습니다 —
병목은 고칠 때마다 옮겨갔습니다.
...
| 무엇 | 합격선 | 왜 이 값 | 어디서 재나 |
|---|---|---|---|
| 입장 경로 p99 | **1초** | 멈추면 눌린 건지 몰라 다시 누름 — 그 재시도가 부하로 더해짐 | queue — `enter` 지연 히스토그램 |
| 현황판 p99 | **3초** | 프론트 폴링이 3초 주기 — 그보다 늦으면 다음 폴이 먼저 옴 | booking — 현황판 응답 지연 |
...
| 회전 — 사람이 실제로 들어가는 속도 | **0보다 큼** | 지연이 멀쩡해도 아무도 못 들어가면 실패 | 승격마다 +1 되는 카운터(Redis)가 늘고 있는가 |
| 메모리 | **limit의 80%** | 수집이 15초 간격 — 그 사이에 튄 봉우리는 지표에 안 찍히니, 찍힌 값에 20% 여유를 둠 | 컨테이너 working_set ÷ limit |
