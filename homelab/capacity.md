...
| **30,000에서 k3s 컨트롤플레인 정지** | **엣지 → traefik TLS 연결 41 → 22,866**<br>origin 응답 3ms → 2.4초 → 엣지가 처리 중 요청 수만큼 연결을 엶<br>1만의 458은 origin이 빨라서<br>연결마다 고루틴·버퍼 → traefik 메모리 limit의 **90%**<br>노드 여유 메모리 2.3GB → **1.1GB**<br>etcd 디스크 쓰기 0.06 → **4초** → k3s 자가 재시작 | **미적용 — 갈래 둘**<br>1. traefik 스펙 상향 — 노드 8GB가 상한<br>2. 동시 요청 상한(`inFlightReq`) — 초과분 즉시 429, 연결 폭발 차단<br>Redis 단일 스레드 한계엔 미도달 |
{:.hl-tbl}
