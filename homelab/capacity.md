...
부하 생성기는 처음에 집 데스크탑에서 쐈습니다. 이 구간의 판들이 선을 넘는 자리 일곱을
잡았고, 값이 하나씩 실측으로 바뀌었습니다. 앞단은 인원이 정하고, 뒷단은 정원이 정합니다.
...
둘이 남았습니다. 연결을 만드는 구간의 실패 1.27%가 열 판 내내 안 내려갔는데 —
서버 · OPNsense · Cloudflare 정책 · k6 자원이 전부 무죄였고, 남은 용의자(집 공유기 ·
엣지)는 둘 다 들여다볼 화면이 없습니다. 그리고 집 장비로는 10,000명 위를 만들 수
없습니다. 가르는 방법은 하나 — **부하 생성기를 집 밖에 두고 같은 판을 돌리는 것**입니다.

## 클라우드에서

(작성 중 — 재실행 판의 실측·스크린샷으로 채웁니다)
