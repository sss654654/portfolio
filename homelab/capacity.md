...
판을 네 번 돌렸고, 마지막으로 30,000 부하로 측정했습니다.
...
| 증상 | 원인 | 조치 |
|---|---|---|
| **오픈 순간 지연이 첫 판에서만 10–20초** | **booking JVM 웜업**<br>파드가 93분 놀아 코드가 인터프리터 상태. 1만 명이 계단으로 들어오자 오픈 2분에 JIT 컴파일 CPU 111초 — 컴파일러가 요청·소비 스레드와 2코어를 경쟁 | **측정 절차에 웜업 판 추가**<br>파드 나이로 거르던 게이트가 유휴 파드에서 뚫림. 데워진 뒤 회차 0.94 · 현황판 0.1초 |
| **데워진 판에서 폴링 셋이 선 밖** | **queue의 Redis 풀이 파드당 10**<br>오픈에 파드당 동시 요청 540이 연결 10개를 대기 — 24,158건 · 10,952초. Redis 자체는 0.34코어로 여유 | **풀 50**<br>대기 93% 감소 · 순번 0.94 · 실황 0.86 · enter 0.93초 |
