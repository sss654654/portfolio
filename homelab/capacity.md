...
| **30,000에서 k3s 컨트롤플레인 정지** | **연결이 메모리를 넘어섬**<br>30,000명을 넣자 엣지 → traefik 연결이 **458 → 22,866**<br>그 연결이 traefik 메모리를 limit의 **90%**, 노드 메모리를 **바닥**까지<br>메모리가 마르자 **k3s 재시작** | **10,000 이상은 메모리가 필요**<br>traefik 메모리 limit과 노드 RAM(8GB × 3대) 상향<br>또는 동시 요청 상한(`inFlightReq`)으로 연결 자체를 막음 — 초과분은 429 |
{:.hl-tbl}
