...
이 판까지 오면서 값 여섯이 바뀌었습니다.

| 값 — 판 전 | 증상 | 원인 | 조치 | 결과 |
|---|---|---|---|---|
| traefik 메모리<br>2대 × 768Mi | 5,000명 부하에서 파드별 워킹셋이 limit의 86% · 82% | 브라우저 연결이 traefik에 사람 수만큼 열림 — 메모리가 인원에 비례 | **3대 × 2Gi** | 10,000명 판에서 40% |
| queue CPU<br>500m | 10,000명 부하에서 스로틀 83% · Redis 연결 포기 초당 59건 | CPU 몫이 모자라 잘리는 동안, queue가 빌린 연결을 못 돌려줌 | **1코어** | 스로틀 0 · 포기 0 |
| booking 메모리<br>1Gi (힙 비율 75%) | 공개 경로 판에서 OOMKill 2회 | 힙 768 + 비힙 222 = 990Mi가 limit 1Gi에 닿음 | **1,536Mi** · 힙 768Mi 고정 | 워킹셋 47% |
| booking CPU<br>1코어 | 정원 500 판에서 스로틀 99.7% | Kafka 소비 스레드 넷이 한 코어에 몰림 | **2코어** | 스로틀 32% |
| DB 커넥션 풀<br>10 | 공개 경로 판에서 풀 대기 397건 | 같은 시각 MySQL은 CPU 12% 유휴 — 상한이 DB가 아니라 풀 | **30** | 대기 0 |
| 승격 배치<br>100명 / 2초 | 오픈 직후 입장 인증 지연 p99 4.97초 | 100명 묶음이 한꺼번에 도착해 booking의 소비가 밀림 | **25명 / 0.5초**<br>(초당 상한 같음) | 0.99초 |
{:.hl-run}
