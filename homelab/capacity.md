...
여기까지 오는 동안 값 일곱이 가정에서 실측으로 바뀌었습니다.

| 스펙 | 가정 → 확정 | 부하가 보여준 것 |
|---|---|---|
| traefik | 2대×768Mi → **3대×2Gi** | 브라우저 연결이 사람 수만큼 열려 메모리가 인원을 따라갑니다. 5,000명 봉우리에서 이미 limit의 86% — 올린 뒤 1만 명 판에서 **40%** |
| queue CPU | 500m → **1코어** | 폴링이 CPU 몫을 다 써 스로틀 83%, 잘린 파드가 Redis 연결을 못 돌려줘 초당 59건이 포기됐습니다 — 올린 뒤 **둘 다 0** |
| booking 메모리 | 1Gi → **1,536Mi** (힙 768Mi 고정) | JVM은 힙 밖에서도 수백 MiB를 씁니다. 힙+비힙 990Mi가 limit에 닿아 OOMKill 2회 — 힙을 고정하고 올린 뒤 워킹셋 **47%** |
| booking CPU | 1코어 → **2코어** | Kafka를 소비하는 스레드 넷이 한 코어를 나눠 써 스로틀 99.7% — 올린 뒤 **32%** |
| 커넥션 풀 | 10 → **30** | 풀 대기 397건인데 MySQL은 CPU 12%로 유휴 — 상한은 DB가 아니라 풀이었습니다. 올린 뒤 대기 **0** |
| 승격 | 100명/2초 → **25명/0.5초** | 100명 묶음이 한꺼번에 도착해 인증 지연 4.97초 — 초당 상한(50명)은 그대로 두고 묶음만 쪼개 **0.99초** |
| 정원 | 2 → **1,000** | 정원을 올려도 자원은 한도의 절반 미만 — 상한은 좌석 4,000이었습니다. 매진 **440초** · 마지막 구매자 대기 **3분 7초** |
{:.hl-cmp}
