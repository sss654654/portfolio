...
|---|---|---|
| **인원 상한 10,000** | 1만 VU가 약 4코어 · 4GB를 써 WSL 메모리가 참 — 30,000명 부하는 못 만듦 | 상한이 **인스턴스 스펙**으로 바뀜 |
| **연결 실패 1.27%** | 실패 내용은 연결 리셋 · 타임아웃. 서버 · OPNsense · Cloudflare 설정 · k6 자원은 지표로 원인이 아님을 확인 — 남은 구간(공유기 · 엣지)은 지표가 없음 | 요청이 공유기를 지나지 않음 — **실패율이 바뀌는지**로 원인 구간이 갈림 |
{:.hl-dec}
