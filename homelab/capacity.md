...
**정원 1,000 · 인원 30,000**까지 걸었습니다. 재는 동안 도구부터 의심했습니다 —
실사용자는 같은 밀리초에 누르지 않으므로 시작·오픈 시각을 흩었고, 집 회선이 서버가
받는 부하를 40% 낮춰 재는 것이 드러나 부하 생성기를 클라우드로 옮겼습니다.
선을 넘는 자리가 나오면 원인을 찾아 값을 올리고, 같은 판을 다시 돌려 선 안에
들어온 것을 확인했습니다 — 올릴 수 있는 상한은 노드 스펙(8GB · 4vCPU × 3대)입니다.
...
| 정상 구간 0.5초 | 전 경로 최고 0.36초 | 통과 |
| 메모리 limit의 80% | 최고 47% (booking 워킹셋) | 통과 |
{:.hl-dec}

초과분의 끝은 입장 인증 등록 9.95초 — 원인을 좁혀 둔 미해결로, 아래 남은 것에 있습니다.
...
- **limit과 request는 다른 구간으로 정했습니다** — 오픈 봉우리와 평시의 실측 비가 2-11배. limit은 봉우리를 견디는 값, request와 파드 수는 평시 값
