...
- **traefik을 2대×768Mi → 3대×2Gi로** 올리자, 10,000명에서 limit에 닿던 메모리(연결이 사람 수만큼 열림 — 사람당 132KiB)가 **40%**로
- **queue CPU를 500m → 1코어로** 올리자, 스로틀 **83% → 0%** · Redis 풀 포기 **초당 59건 → 0건**
- **booking 메모리를 1Gi → 1,536Mi(힙 768Mi 고정)로** 올리자, OOMKill 2회(힙+비힙 990Mi)가 **워킹셋 47%**로
- **booking CPU를 1코어 → 2코어로** 올리자, 스로틀 **99.7% → 32%** — 원인은 예상 밖: Kafka 소비 스레드 넷이 한 코어에 몰려 있었음
- **커넥션 풀을 10 → 30으로** 올리자, 대기 **397건 → 0건** — 그 순간 MySQL은 CPU 12%로 유휴: 상한은 DB가 아니라 풀이었음
- **승격을 100명/2초 → 25명/0.5초로** 바꾸자, 인증 지연 **4.97초 → 0.99초** — 초당 상한(50명)은 같고 묶음 크기만 4분의 1
- **정원은 2 → 1,000** — 자원이 아니라 좌석 4,000이 상한: 더 올리면 100초에 소진돼 잴 구간이 없음
