...
둘 사이는 **Kafka 비동기 메시지**로만 잇습니다. 두 서비스를 묶는 값이 `정원` —
동시에 들여보낼 인원입니다. **예매 오픈**을 눌러 보세요 — 한 판이 돕니다.
...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| queue | **Go · 4대** — 늘려서 나누는 쪽 | **요청이 짧고 많음 — 대기열**<br>몇 명이 몰릴지 알 수 없는 자리. 요청 하나를 goroutine(수 KB)이 맡아 동시에 많이 받는 비용이 작고, 빌드 때 이미 기계어라 예열 없이 첫 요청부터 같은 속도 — 파드를 늘리면 다음 요청부터 바로 나뉨 |
| 순번·현황 | **Redis에 묻는다(폴링)** | **홈과 대기 화면이 주기마다 묻는 구조**<br>물음의 답(줄·정원·현황)이 전부 Redis에 있어 요청이 왕복 한두 번(1ms)에 끝남 — 부하가 횟수(CPU)로 오고, 어느 queue 파드가 받아도 같은 답 |
| booking | **Java Spring · 한 대** | **요청이 길고 적음 — 입장객**<br>MySQL을 기다리는 동안 메모리를 쥐고, 수는 정원으로 묶임. 돈·좌석의 전부-성공/전부-롤백 경계를 선언(`@Transactional`) 하나로 걸 수 있어 자바 — 대가는 시작부터 큰 메모리 |
| 서비스 간 통신 | **Kafka 비동기** — 서로 직접 부르지 않음 | **한쪽이 죽어도 다른 쪽이 안 막힘**<br>동기 호출이면 booking이 죽는 순간 queue까지 같이 막힘. Kafka는 발행하고 끝 — 못 받은 것은 토픽에 남아 되살아난 뒤 따라잡음 |
{:.hl-dec}

## 앱 대시보드

부하를 걸기 전에 볼 눈부터 만들었습니다 — 막힐 거라 예상해 둔 자리(파드 자원·커넥션 풀·
Redis·MySQL·좌석 재고)의 **절반 넘게 지표가 없었습니다.** 없던 배선을 앱에 심고
...
축 둘을 차례로 올렸습니다 — **인원**(줄에 몇 명이 서나)과 **정원**(동시에 몇 명이 예매하나).
병목은 고칠 때마다 다른 곳으로 옮겨갔고, 아래가 그 기록입니다.

| 판 | 나온 것 | 정한 것 |
|---|---|---|
| 버린 네 판 — 인원 1,000부터 | 잰 값이 전부 과소 — 앞단 traefik이 먼저 잘리고 있었고, 부하 도구의 도착 시각이 정렬돼 있었음 | **앞을 열고 도착을 흩은 판의 값만 쓴다** |
| 인원 계단 1,000 → 5,000 · 정원 2 고정 | traefik 메모리·queue CPU·Redis 명령이 인원에 정비례 — 한 명이 늘 때 드는 양이 직선으로 나옴 | **사람당 계수** |
...
풀 대기 0 · 전원 완주. 인원만 5배 올려도 booking은 그대로였습니다 — 줄이 아무리 길어도
**뒤가 안 밀립니다.**

값보다 오래 남는 것은 **계수**입니다 — 사람 한 명·확정 한 건이 쓰는 자원의 기울기라,
안 재 본 목표도 계산이 됩니다. "좌석 4,000을 10분에 팔겠다"를 넣으면
booking CPU 18% · MySQL 7%가 나옵니다.
...
- **3만 명 위는 못 쟀습니다** — 노드 메모리(8GB×3)의 물리 한계입니다. 자원이 큰 클러스터에서 같은 방법으로 다시 잽니다
