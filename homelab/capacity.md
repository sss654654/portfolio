...
k6가 실제 여정(입장 → 대기 → 좌석 → 확정) 그대로 부하를 만듭니다. 집 경로에서
**인원 1,000 → 10,000**(정원 2 고정) → **정원 2 → 500**, 공개 후에는 인터넷 경로에서
**정원 1,000 · 인원 30,000**까지 걸었습니다. 선을 넘는 자리가 나오면 원인을 찾아 값을
올리고, 같은 판을 다시 돌려 선 안에 들어온 것을 확인했습니다 — 올릴 수 있는 상한은
노드 스펙(8GB · 4vCPU × 3대)입니다.

## 결과

<div class="hl-sub" markdown="0">합격선 판정 — 최종 판 (공개 경로 · 인원 10,000 · 정원 1,000)</div>

| 합격선 | 실측 | 판정 |
|---|---|---|
| 5xx 0건 | 0건 / 759,813 요청 — 확정 실패 0 · 재시작 0 · 전원 여정 완주 | 통과 |
| 입장 경로 1초 | 평상 0.36초 · 오픈 순간 3.76초 | 오픈 30초만 초과 |
| 현황판 3초 | 41ms — 엣지 캐시 5초 뒤의 origin | 통과 |
| 정상 구간 0.5초 | 전 경로 밀리초 — 최고 0.36초 | 통과 |
| 메모리 limit의 80% | 기록 최고 47% (booking 워킹셋) | 통과 |
{:.hl-dec}

초과분의 끝은 입장 인증 등록 9.95초입니다 — 원인 다섯을 지워 "Kafka 리스너 한 호출
4.92초"까지 좁혔고, 아래 남은 것에 있습니다.

<div class="hl-sub" markdown="0">확정 스펙 — 선을 넘게 한 것이 값을 정했다</div>

| 스펙 | 가정 → 확정 | 선을 넘게 한 것 → 올린 뒤 |
|---|---|---|
| traefik | 2대×768Mi → **3대×2Gi** | 연결이 사람 수만큼 열림 — 5,000명 봉우리에 파드가 limit의 86% → **10,000명에서 40%** |
| queue CPU | 500m → **1코어** | 몫을 다 써 스로틀 83% · Redis 풀 포기 초당 59건 → **둘 다 0** |
| booking 메모리 | 1Gi → **1,536Mi** (힙 768Mi 고정) | 힙+비힙 990Mi가 limit에 닿아 OOMKill 2회 → **워킹셋 47%** |
| booking CPU | 1코어 → **2코어** | Kafka 소비 스레드 넷이 한 코어에 몰림 — 스로틀 99.7% → **32%** |
| 커넥션 풀 | 10 → **30** | 대기 397건인데 MySQL은 CPU 12% 유휴 — 상한은 DB가 아니라 풀 → **대기 0** |
| 승격 | 100명/2초 → **25명/0.5초** | 묶음 100명이 인증 지연 4.97초를 만듦(초당 상한 50은 같음) → **0.99초** |
| 정원 | 2 → **1,000** | 어느 자원도 한도의 절반 미만 — 상한은 좌석 4,000 → **매진 440초 · 마지막 구매자 대기 3분 7초** |
{:.hl-cmp}

- **인원만 5배 올려도 booking은 안 움직였습니다** — 대기열이 정원만큼만 흘려보내는 구조가 실측으로 확인된 것
- **단위당 소모량도 실측됐습니다** — 사람 1명·확정 1건이 쓰는 자원의 양. 목표를 넣으면 스펙이 나옵니다: "좌석 4,000을 10분에"(확정 6.7건/초) → booking CPU 18% · MySQL 7%

## 남은 것

- **오픈 순간의 인증 등록 지연 9.95초** — 원인 다섯을 지워 "Kafka 리스너 한 호출 4.92초"까지 좁혔습니다. 더 가려면 스레드 덤프가 필요합니다
- **Redis만 자원으로 못 풉니다** — 단일 스레드라 코어를 더 줘도, 파드를 늘려도 그 한 대가 나뉘지 않습니다. 단위당 소모량으로 계산하면 약 2만 5천 명에서 1코어 — 그 위는 명령 수를 줄이는 코드 변경이나 샤딩입니다
