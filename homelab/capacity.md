...
|---|---|---|
| **인원 상한 10,000** | 1만 VU가 약 4코어 · 4GB를 써 WSL 메모리가 참 — 이 장비는 10,000이 끝 | 초기 목표에 도달하기 위해 10,000 다음 단계를 만들려면 상한이 **인스턴스 스펙**이 되어야 함 |
| **연결 실패 1.27%** | 실패 내용은 연결 리셋 · 타임아웃. 서버 · OPNsense · Cloudflare 설정 · k6 자원은 지표로 원인이 아님을 확인 — 남은 구간(공유기 · 엣지)은 지표가 없음 | 생성기 위치를 바꿔 **원인 구간을 규명** |
{:.hl-dec}
