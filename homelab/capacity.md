...
옮겨 공개 경로에서 30,000까지 걸었습니다. 한 판에 한 변수 — 가정 값이 걸릴 때마다
아래 값이 하나씩 실측으로 바뀌었습니다.

| 값 | 가정 → 잰 값 | 부하가 드러낸 것 → 확인 |
|---|---|---|
| traefik | 2대 × 768Mi → **3대 × 2Gi** | 연결이 사람 수만큼 열려(사람당 132KiB) 10,000명에서 메모리가 참 → 올린 뒤 40% |
| queue CPU | 500m → **1000m** | 10,000명에서 스로틀 83% · 풀 포기 초당 59건 → 0% · 0건 |
| booking CPU | 1코어 → **2코어** | Kafka 소비 스레드 넷이 1코어에 올라 스로틀 99.7% → 32%, 최종 판 0% |
| booking 메모리 | 1Gi → **1,536Mi** · 힙 768Mi 고정 | 힙+비힙 990Mi가 limit에 닿아 OOMKill 2회 → 워킹셋 47% |
| DB 커넥션 풀 | 10 → **30** | 대기 397건인데 MySQL은 CPU 12%로 유휴 — 상한이 풀이었음 → 대기 0 |
| 승격 배치 / 주기 | 100명 / 2초 → **25명 / 0.5초** | 뭉텅이 100명이 인증 지연 4.97초를 만듦(초당 상한 50은 동일) → 0.99초 |
| 동시 입장 정원 | 2 → **1,000** | 자원은 안 걸림 — 상한은 좌석 4,000: 더 올리면 100초에 소진돼 잴 구간이 없음 |
{:.hl-cmp}
