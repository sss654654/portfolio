...
| 증상 | 원인 | 조치 |
|---|---|---|
| **오픈 순간 지연이 첫 판에서만 10–20초** | **booking JVM 웜업**<br>93분 유휴 파드 = 코드가 인터프리터 상태<br>오픈 2분에 JIT 컴파일 CPU **111초**<br>컴파일러가 2코어를 요청·소비 스레드와 경쟁 | **측정 절차에 웜업 판 추가**<br>파드 나이 게이트는 유휴 파드에서 무효<br>데워진 뒤 회차 0.94초 · 현황판 0.1초 |
| **데워진 판에서 폴링 셋이 선 밖** | **queue Redis 풀 파드당 10**<br>오픈에 파드당 동시 요청 540 → 연결 10개 대기<br>대기 **24,158건 · 10,952초**<br>Redis 자체는 0.34코어로 여유 | **풀 50**<br>대기 93% 감소<br>순번 0.94 · 실황 0.86 · enter 0.93초 |
| **30,000에서 k3s 컨트롤플레인 정지** | **엣지가 연 연결이 traefik 메모리를 넘어섬**<br>1. 오픈에 30,000명 동시 폴링 → 초당 요청 10,000<br>2. origin 응답 3ms → **2.4초**<br>3. 엣지는 처리 중인 요청마다 연결 하나 → 연결 **41 → 22,866**<br>4. traefik은 연결마다 고루틴·버퍼 → 메모리 limit의 **90%**<br>5. 노드 여유 메모리 2.3GB → **1.1GB** → 전 프로세스 지연<br>6. etcd 디스크 쓰기 0.06 → **4초** → k3s 재시작<br>(10,000은 응답 3ms라 연결 458에서 끝남) | **10,000 이상은 스펙 상향이 필요**<br>traefik 메모리 limit · 파드 수 — 연결 2만을 들 수 있게<br>노드 RAM — 8GB × 3대가 상한, 홈랩 밖<br>또는 동시 요청 상한(`inFlightReq`) — 연결 폭발 자체를 차단, 초과분은 429<br>전부 미적용 |
{:.hl-tbl}
