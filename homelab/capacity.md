...
<div class="hl-shots" markdown="0" aria-label="Redis·Kafka 대시보드 — 화살표로 넘겨 봅니다">
  ...
  <figure class="hl-shot">
    ...
    → 이 판: 승격→인증 <b>1초 아래</b> · 확정→반환 0.1초.<br>
    → <b>클라우드 판에서 다시 2.5–5초로 돌아왔습니다</b> — 묶음 크기가 원인의 전부는 아니었고,
    남은 원인은 아직 못 짚었습니다.</figcaption>
  </figure>
</div>
...
| **데워진 판에서 폴링 셋이 선 밖** | **queue Redis 풀 파드당 10**<br>오픈에 파드당 동시 요청 540 → 연결 10개 대기<br>대기 **24,158건 · 10,952초** | **풀 50**<br>대기 10,952초 → **785초**<br>순번 **0.94초** · 실황 **0.86초** · enter **0.93초** |
