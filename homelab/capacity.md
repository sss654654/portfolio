...
앞단은 인원이 걸었고, 뒷단은 정원이 걸었습니다.

- **traefik 2대×768Mi → 3대×2Gi** — 연결이 사람 수만큼 열려(사람당 132KiB) 10,000명에서 메모리가 참 → 올린 뒤 40%
- **queue CPU 500m → 1코어** — 정한 것대로 CPU가 걸림: 스로틀 83% · 풀 포기 초당 59건 → 둘 다 0
- **booking 1Gi → 1,536Mi(힙 768Mi 고정) · 1코어 → 2코어** — 정한 것대로 메모리가 걸렸고(힙+비힙 990Mi가 닿아 OOMKill → 워킹셋 47%), CPU는 예상 밖 — Kafka 소비 스레드 넷이 한 코어에 몰려 99.7% → 32%
- **커넥션 풀 10 → 30** — 대기 397건인 순간 MySQL은 CPU 12%로 유휴: 상한은 DB가 아니라 풀 → 대기 0
- **승격 100명/2초 → 25명/0.5초** — 문제는 양이 아니라 뭉텅이: 초당 상한(50명)은 그대로, 인증 지연 4.97 → 0.99초
- **정원 2 → 1,000** — 자원은 안 걸림: 좌석 4,000이 먼저 끝남 — 더 올리면 100초에 소진돼 잴 구간이 없음
