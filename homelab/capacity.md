...
| 증상 | 원인 | 조치 |
|---|---|---|
| **오픈 순간 지연이 첫 판에서만 10–20초** | **코드가 컴파일되기 전에 부하가 옴**<br>자바는 실행하면서 컴파일하는데, booking이 93분 놀아 아직 컴파일 전<br>오픈에 1만 명이 오자 컴파일이 CPU **111초**를 먹어 요청 처리는 그만큼 지연 | **측정 절차에 웜업 판 추가**<br>파드 나이 게이트는 유휴 파드에서 무효<br>데워진 뒤 회차 0.94초 · 현황판 0.1초 |
