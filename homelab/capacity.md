...
부하는 k6가 실제 여정(입장 → 대기 → 좌석 → 확정) 그대로 만듭니다.
...
- **공개 경로 · 정원 1,000 → 인원 30,000** — 격리와 공개 작업 뒤, 실사용과 같은 인터넷 경로에서 다시 잽니다

## 결과

최종 판은 **공개 경로 · 인원 10,000 · 정원 1,000 · 좌석 4,000**입니다. 판정 다섯 중
넷이 통과했고, 입장 경로 하나가 오픈 구간에서 선을 넘었습니다.

| 합격선 | 최종 판 실측 | 판정 |
|---|---|---|
| 5xx 0건 | 759,813 요청 중 0건 — 확정 실패 0 · 파드 재시작 0 · 전원 완주 | 통과 |
| 입장 경로 1초 | 순번 조회 p99 — 평상 0.36초 · 오픈 시 3.76초 | 오픈 구간만 초과 |
| 현황판 3초 | p99 41ms — 요청 대부분은 엣지 캐시가 받음 (origin 도착 90초에 26건) | 통과 |
| 정상 구간 0.5초 | 여섯 경로 p99 53–362ms | 통과 |
| 메모리 limit의 80% | booking 워킹셋 718Mi / limit 1,536Mi = 47% | 통과 |
{:.hl-dec}

오픈 구간 초과의 가장 큰 값은 입장 인증 등록 9.95초입니다 — 원인을 좁혀 둔 미해결로,
남은 것에 적었습니다.

여기까지 오는 동안 값 일곱이 바뀌었습니다. 앞단은 인원이 정하고, 뒷단은 정원이 정합니다.
...
| queue CPU · 500m | 스로틀 83% · Redis 연결 포기 초당 59건 | CPU가 잘리는 동안 queue가 빌린 연결을 못 돌려줘, 뒤따라온 요청이 포기함 | **1코어** | 스로틀 0 · 포기 0 |
| 승격 · 100명 / 2초 | 오픈 직후 입장 인증 지연 4.97초 | 100명이 한 묶음으로 도착해 booking의 인증 소비가 밀림 | **25명 / 0.5초** | 0.99초 (초당 상한은 그대로) |
{:.hl-run}

<div class="hl-sub" markdown="0">뒷단 — 정원을 올려 잰다 (정원 1,000 판)</div>

| 값 · 판 전 | 부하가 드러낸 것 | 왜 그런가 | 올린 값 | 올린 뒤 |
|---|---|---|---|---|
| booking 메모리 · 1Gi | OOMKill 2회 | 힙 768Mi 밖에서 JVM이 비힙 222Mi를 더 써, 합 990Mi가 limit 1Gi에 닿음 | **1,536Mi · 힙 768Mi 고정** | 워킹셋 47% |
| booking CPU · 1코어 | 스로틀 99.7% | Kafka를 읽는 소비 스레드 넷이 코어 하나에 몰림 | **2코어** | 스로틀 32% |
...
정원의 상한은 자원이 아니라 좌석 4,000이었습니다 — 그보다 올리면 좌석이 100초 안에 소진돼 잴 구간이 없어집니다.

## 남은 것

- **오픈 순간의 입장 인증 등록이 9.95초까지 늘어집니다** — 승격 묶음을 쪼개 0.99초로 잡았던 그 지연입니다. 부하 생성기를 집에서 클라우드로 옮기자 서버에 닿는 부하가 커졌고(집 회선이 부하를 40% 낮춰 재고 있었습니다), 그 조건의 최종 판에서 다시 선을 넘었습니다. 원인 다섯을 배제해 "Kafka 리스너 한 호출 4.92초"까지 좁혔고, 다음 단계는 스레드 덤프입니다
...
- **확정 1건당 자원 소모(booking CPU · MySQL 질의)는 확정 2-11건/초 구간에서 실측한 값입니다** — 26.5건/초에서 검산하니 오차 19%였습니다. 그 위 속도는 외삽이라, 목표 처리량이 거기 걸리면 다시 재야 합니다
