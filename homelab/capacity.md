...
- **점진 부하** — 인원과 정원을 계단으로 올립니다
  ...
  - 아래 캐러셀의 **"이 판"은 마지막 계단** — 인원 10,000 · 정원 1,000입니다
...
| **연결 실패 1.27%** | 실패 내용은 연결 리셋 · 타임아웃.<br>서버 · OPNsense · Cloudflare 설정 · k6 자원은 지표로 원인이 아님을 확인 — 남은 구간(공유기 · 엣지)은 지표가 없음 | 생성기 위치를 바꿔 **원인 구간을 규명** |
{:.hl-dec}

## 부하테스트(클라우드)

Terraform으로 띄운 인스턴스(8vCPU · 16GB)에서 같은 스크립트를 쐈습니다 — 인원 10,000 · 정원 1,000.
판을 네 번 돌렸고, 아래 판정은 마지막 판입니다.
...
| 전달 기준선 2초 | 승격→인증 **942건(12.6%)이 2.5–5초** — 5초 초과 0, 전부 오픈 직후 | **초과** |
{:.hl-dec}

k6 실패율 1.34%는 전부 생성기와 엣지 사이 **연결 수준**(리셋 · 타임아웃)이고, 서버가 낸 5xx는 0입니다.
생성기를 옮겨도 이 실패가 남았으므로 **공유기는 원인이 아닙니다.** RST를 보낸 쪽은 엣지 주소이고, 서버는
요청을 다 받았고(traefik 진입 471,417건 = 캐시 제외분), 포트 고갈이면 오류 종류가 다릅니다(`cannot assign
requested address`). 남는 조건은 **단일 출발지 IP에서 동시 연결 1만 개** — 연결 수에 비례해 늘고, 실사용자는
IP당 연결 하나라 이 조건 자체가 성립하지 않습니다. 엣지 안에서 어느 규칙이 끊는지까지는 못 봤습니다.

| 판에서 드러난 것 | 원인 | 조치 · 상태 |
|---|---|---|
| **오픈 순간 지연이 첫 판에서만 10–20초**<br>회차 4.8초 · 현황판 2.4초 · 승격→인증 10–20초 구간 | **booking JVM 웜업**<br>파드가 93분 놀아 코드가 인터프리터 상태. 1만 명이 계단으로 들어오자 오픈 2분에 JIT 컴파일 CPU 111초 — 컴파일러가 요청·소비 스레드와 2코어를 경쟁 | **측정 절차에 웜업 판 추가**<br>파드 나이로 거르던 게이트가 유휴 파드에서 뚫림. 데워진 뒤 회차 0.94 · 현황판 0.1초 |
| **데워진 판에서 폴링 셋이 선 밖**<br>순번 4.05 · 실황 4.30 · enter 2.49초 | **queue의 Redis 풀이 파드당 10**<br>오픈에 파드당 동시 요청 540이 연결 10개를 대기 — 24,158건 · 10,952초. Redis 자체는 0.34코어로 여유 | **풀 50**<br>대기 93% 감소 · 순번 0.94 · 실황 0.86 · enter 0.93초 |
| **승격→인증 942건이 2.5–5초** | **리스너 스레드 넷이 1.5–2.6초씩 정지**<br>처리는 30ms, 발행은 초당 20건이라 처리량 문제가 아님. 판마다 JIT 증가와 함께 줄어 웜업 잔여가 유력 | **미확정**<br>배치 소비 스팬에 자식이 없어 정지 구간을 못 봄 — 스팬 추가가 다음 |
| **30,000에서 컨트롤플레인 정지**<br>API 거부 · MetalLB 주소 소멸 · 프로브 연쇄 실패 · **5xx 첫 발생** — k3s-1의 queue 한 대가 503 4,477건 | **엣지가 처리 중인 요청 수만큼 origin 연결을 연다**<br>한 분 안에 origin 응답 0.003 → 2.4초, 앞 연결 41 → 22,866(≈ 초당 도착 10,000 × 2.4초). 1만에서 458이었던 건 origin이 빨라서지 엣지가 접어 줘서가 아님. 연결이 traefik 메모리 limit 90%·고루틴 84,000을 먹고 → 노드 여유 메모리 2.3GB → 1.1GB(2분) → 커널 회수 major fault 30/초 → load 85 → **etcd fsync p99 0.06 → 4.02초**(디스크 io 3% — 프로세스가 스케줄을 못 받음) → k3s 재시작. 느려질수록 연결이 느는 되먹임 | **첫 벽은 Redis가 아니라 origin 동시 요청 수**<br>예측(Redis 단일 스레드)에 닿기 전에 진입 노드가 무너짐. 손잡이는 엣지가 아니라 origin 쪽 — 동시 요청 상한(traefik inFlightReq)으로 초과분을 즉시 429로. 미적용 |
{:.hl-tbl}
