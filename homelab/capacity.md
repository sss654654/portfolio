...
- **30,000에서 k3s 컨트롤플레인 정지** — 엣지 → traefik 연결이 **458 → 22,866**. 그 연결이 traefik 메모리를 limit의 **90%**, CPU를 **스로틀 30%**, 노드 메모리를 **바닥**까지. 메모리가 마르며 etcd 디스크 쓰기가 **4초**로 늘고 k3s 재시작. 10,000 위로 가려면 traefik 메모리·CPU limit과 노드 RAM(8GB × 3대) 상향이 선행
- **엣지가 연결을 대신 받는 구조가 30,000에서 성립하지 않음** — 이유 미규명
