...
| **30,000에서 컨트롤플레인 정지**<br>API 거부 · MetalLB 주소 소멸 · 프로브 연쇄 실패 · **5xx 첫 발생** — k3s-1의 queue 한 대가 503 4,477건 | **엣지 뒤 연결이 458 → 23,000**<br>인원 3배에 연결 50배. traefik 메모리 limit 85% · 고루틴 84,000 → 노드 여유 메모리 2.3GB → 1.1GB(2분) → 커널 회수로 major fault 30/초 → load 85 → **etcd fsync p99 0.06 → 4.02초**(디스크 io 3%, 프로세스가 스케줄을 못 받음) → etcd 불안정 → k3s 재시작 | **첫 벽은 Redis가 아니라 노드 메모리**<br>예측(Redis 단일 스레드 1코어)에 닿기 전에 진입 노드가 먼저 무너짐. 엣지가 연결을 접어 주는 상한이 1만과 3만 사이 — 그 지점은 못 잼 |
{:.hl-tbl}
