...
부하는 k6 — 가상 사용자가 실제 여정(줄서기 → 폴링 → 좌석 → 확정)을 그대로 밟습니다.
**인원**을 1,000 → 10,000으로(정원 2 고정), 다음 **정원**을 2 → 500으로(인원은 정원을
채울 만큼만), 마지막엔 생성기를 클라우드로 옮겨 공개 경로에서 다시 — 30,000명에서
노드 메모리(물리 한계)에 닿아 멈췄습니다. 한 판에 한 변수만 바꿨고, 병목은 고칠 때마다
옮겨갔습니다 — 옮겨간 자리와 고친 값이 아래 결과입니다.

## 결과

왼쪽 열은 전부 가정 값이었고, 오른쪽 열은 전부 잰 값입니다.

| 값 | 판 전 | 판 후 | 근거 |
|---|---|---|---|
| 동시 입장 정원 | 2 — 데모값 | **1,000** | 상한은 자원이 아니라 좌석 4,000 — 더 올리면 100초에 소진돼 잴 구간이 없음 |
| 세션 | 60초 | **300초** | 여정 실측 합 100-330초 |
| DB 커넥션 풀 | 10 | **30** | 10일 때 대기 397건(같은 시각 MySQL은 CPU 12%로 유휴) → 30에서 대기 0 |
| booking 메모리 | 1Gi | **1,536Mi** · 힙 768Mi 고정 | 힙+비힙 990Mi가 limit에 닿아 OOMKill 2회 → 1,536Mi에서 워킹셋 47% |
| booking CPU | 1코어 | **2코어** | 1코어에 Kafka 소비 스레드 넷이 올라 스로틀 99.7% → 2코어에서 32%, 최종 판 0% |
| queue CPU | 500m | **1000m** | 10,000명에서 스로틀 83% → 1000m에서 0% |
