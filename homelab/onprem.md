...
<figcaption>세 경로 모두 노트북 안 OPNsense VM 경유 — 키 없는 VPN 요청은 무응답.</figcaption>
</figure>
...
| 항목 | 선택 | 이유 |
|---|---|---|
| 배포판 | **k3s** — 3대 모두 control-plane · etcd | 8GB 노드에 서비스 몫을 남기는 단일 바이너리. 대가 — 컨트롤 플레인과 kubelet이 한 프로세스라 동반 종료 |
...
| 증상 | 원인 | 조치 |
|---|---|---|
| 443에 `Host` 헤더만 바꿔 요청하면 **ArgoCD 로그인 화면이 200으로 응답** | Traefik이 `Host` 헤더로만 라우팅 — 예매 · Grafana · ArgoCD가 같은 80 · 443 뒤에 위치 | 관리 UI 라우터를 **80 전용**으로 분리 — 80은 인터넷 미개방 |
