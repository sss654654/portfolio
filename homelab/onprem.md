...
| 항목 | 선택 | 이유 |
|---|---|---|
| 배포판 | **k3s** — 3대 모두 control-plane · etcd | 단일 바이너리라 8GB 노드에서 서비스 메모리 확보. 대가 — 컨트롤 플레인과 kubelet이 한 프로세스라 함께 종료 |
...
| 노드가 신고한 **가용 메모리가 실제보다 큼** | 컨트롤 플레인이 파드가 아닌 프로세스라 kubelet 집계에서 빠짐 — 빠진 사용량 **1,783Mi** | 예약값 지정(k3s 2Gi · OS 512Mi) → 노드당 파드 할당 **5,081Mi** |
