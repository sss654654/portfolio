...
노드는 **방화벽 뒤 격리망**에 두고, 서비스는 Cloudflare를 거쳐 공개(443) · 관리 화면은 WireGuard VPN으로만 접근(51820).
{:.lead}
...
<figcaption>세 경로 모두 노트북 안 OPNsense VM 경유 — Cloudflare 대역 밖 443은 차단, 키 없는 VPN 요청은 무응답.</figcaption>
</figure>
...
| 항목 | 선택 | 이유 |
|---|---|---|
| 배포판 | **k3s** — 3대 모두 control-plane · etcd | 8GB 노드에 서비스 몫을 남기는 단일 바이너리 · 1대 중단에도 etcd 과반 유지. 대가 — 컨트롤 플레인과 kubelet이 한 프로세스라 동반 종료 |
...
| 스토리지 | **정적 PV** — 노드 디스크 직접 연결 | 기본 local-path는 한 파일시스템의 폴더 — 디스크 지표로 사용 주체 식별 불가. 대가 — 파드가 노드에 고정 |
{:.hl-dec}
...
| 공개 경로 | **Cloudflare 프록시** + 방화벽 출발지를 엣지 대역으로 제한 | 집 공인 IP 은닉 · 방문자 연결은 엣지에서 종료 |
...
| 증상 | 원인 | 조치 |
|---|---|---|
| 443에 `Host` 헤더만 바꿔 요청하면 **ArgoCD 로그인 화면이 200으로 응답** | Traefik이 `Host` 헤더로만 라우팅 — 예매 · Grafana · ArgoCD가 같은 80 · 443 뒤에 위치 | 관리 UI 라우터를 **80 전용**으로 분리 — 443은 예매 화면만, 80은 인터넷 미개방 |
| 노드가 신고한 **가용 메모리가 실제보다 큼** | 컨트롤 플레인이 파드가 아닌 프로세스라 kubelet 집계에서 누락 — **1,783Mi** | 예약값 지정(k3s 2Gi · OS 512Mi) → 노드당 파드 할당 **5,081Mi** |
| 데스크탑 → 격리망 **응답 없음**, 방화벽 로그는 pass | WAN 규칙에 자동으로 붙은 `reply-to`가 같은 대역 응답을 공유기로 보냄 | 해당 규칙 `reply-to` 해제 → 손실 **0%** |
{:.hl-tbl}

## 결과

- **3대 모두 control-plane · etcd** — 1대 중단에도 클러스터 유지
- **인터넷 개방 포트 2개** — `443/TCP` 서비스 · `51820/UDP` 관리 터널, 외부 포트 20개 스캔 응답 없음
- **파드 간 필요 경로만 허용** — booking 출구는 MySQL · Redis · Kafka · 수집기 4곳
- **설치 · 부트스트랩 · 앱 모두 코드** — k3s `config.yaml` · 부트스트랩 9단계 · Helm 차트, 같은 정의로 stg 구축

## 한계

- **관리 경로가 OPNsense 1대에 집중** — VM 중단 시 터널 · 격리망 인터넷 동시 중단
- **Proxmox · VM 생성은 수동** — 코드는 k3s부터, stg는 Terraform
- **관리용 도메인 1개가 집 공인 IP 노출** — 엣지 프록시는 UDP 터널을 중계하지 않음
