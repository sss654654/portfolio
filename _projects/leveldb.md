...
단국대학교 시스템 소프트웨어 연구실 학부 연구생으로 참여해(2022.07–12) LevelDB의 캐시 구조를 분석하고,
결과를 **KSC 2022 학부생 논문**으로 게재했습니다.

LevelDB는 Google이 만든 임베디드 key-value 저장소입니다. 기반인 LSM-tree는 변경을 메모리에 모았다가
디스크로 순차 기록해 쓰기가 빠른 대신, 찾는 key가 어느 SSTable에 있는지 몰라 여러 파일을 뒤져야 해서
읽기가 약합니다. 그 약점을 **인덱스 캐시와 블록 캐시** 두 계층으로 보완합니다.

맡은 것은 그 두 캐시입니다. 소스를 열어 보니 키도 저장물도 다른 두 캐시가
**LRU 엔진 하나를 공유하는 구조**였고, 여기서 세 가지를 물었습니다 —
두 캐시는 각각 무엇을 담나, 공유 엔진은 어떻게 O(1) 조회와 동시 접근을 지키나,
파라미터를 어떻게 잡아야 읽기가 빠른가. 아래 세 절이 그 순서입니다.
...
- **한 노드로 조회와 순서를 함께** — 보통 서로 다른 자료구조를 요구하는데, 같은 `LRUHandle`을 해시테이블과 이중 연결 리스트에 동시에 걸어 한 번에 만족시킵니다
