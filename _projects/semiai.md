...
| 프로필 | **Pyroscope 연속 프로파일링** — Alloy가 `/debug/pprof`를 주기 수집, MinIO 저장 | OOMKilled는 SIGKILL이라 trace·log가 flush 전에 끊김 — 죽기 직전 힙을 함수별로 보려면 계속 찍고 있어야 함 |
...
- **힙 프로필로 메모리를 점유하는 함수를 찾아 개발팀에 공유했습니다** — 개발자가 로직을 수정한 뒤 메모리 78% 감소, 50x 에러 소멸
