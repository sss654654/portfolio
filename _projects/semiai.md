...
| 무엇을 | 고른 것 | 그렇게 한 이유 |
|---|---|---|
| 수집기 | **Alloy 하나로 통합** | 신호마다 수집기를 따로 두던 것(Prometheus · Promtail · OTel Collector · Pyroscope agent)을 한 설정으로 |
...
| 네 신호 연결 | **`trace_id`로 잇고 metric→trace만 dataLink** | 한 신호에서 나머지로 바로 건너감 · 라벨에 `trace_id`를 넣으면 요청마다 시계열이 폭발 |
...
| 증상 | 원인 | 조치 |
|---|---|---|
| master 2대를 정지시켰는데 대시보드는 **Ready(정상)** | 노드 상태가 kube-state-metrics를 거쳐 오는데, 갱신이 멈춘 채 옛 값으로 남음 | kubelet `:10250`을 Alloy가 직접 scrape한 `up`으로 판정 — etcd도 `etcd_server_has_leader` self-metric으로 |
